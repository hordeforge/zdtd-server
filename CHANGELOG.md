...
- **Blocks whose material disallows support stop holding up structures.** `material_stability_support` was parsed but its fallback ran only for blocks with no resolved MaxDamage, so 80 stock blocks (cloth luggage, organic carcasses, leather, plants, land mines) reported support=true where stock's `BlocksFromXml` (IL_06DE-070F) takes the material's false. The fill now runs for every block whose own properties are silent, and `cntLuggageMediumOpen` / `goreBlockAnimal` pin it.

### Fixed

- **A wandering horde takes its size and entity group from the gamestage ladder.** Stock's `AIWanderingHordeSpawner` builds a party spawner on the `WanderingHorde` ladder in `gamestages.xml` with `mod = 50` (`AIWanderingHordeSpawner::.ctor` IL_0066; the ladder's own comment says "These will wrap around at 50") and spawns the resolved stage row's `group` `num` times (`UpdateSpawn` IL_006D), with the ladder index coming from `GameStageDefinition::CalcGameStageAround` - the weighted party level of the players within 100 m. zdtd spawned a fixed `wandering_horde_size` of the biome's day/night group, so a stage-30 pack had the wrong count and the wrong zombie mix on the wire. The pack now resolves the ladder row at `party stage % 50` through the same `stage_group_fn` projection the blood moon uses (falling back to the rule-sized biome pack when no ladder is wired, as in an offline or builtin world), and the stage comes from a new `stage_around_fn` hook wired to `partyStageAround`, so a two-player server is not gated by its single highest member.

- **Generated columns use the biome's own layer stack.** Stock writes every `<subbiome>` before the biome's own `<layers>`, and the loader took the first `<layers>` in the body - so `burnt_forest` picked up subbiome 0's extra `Depth 1 terrStone` layer instead of its own four and `wasteland` painted `terrDestroyedGrass` where its own stack says `terrDestroyedStone,terrDestroyedGrass`. Every chunk column in those two biomes was wrong on the wire. The loader now selects the `<layers>` group that is not inside a `<subbiome>` (and a biome without one, like `underwater`, keeps the empty-stack path instead of borrowing a subbiome's). The same pass raises the distant-deco cap from 12 to 16 (and the wire-side species capacity with it, asserted equal at build time): stock's richest group is `pine_forest`'s "Dense large trees" subbiome with 14 surviving rows, so `treeDeadTree01` and `treeMountainPine12m` were being dropped.

- **Vehicle health comes from the placeable item, not a literal.** `Vehicle::SetItemValue` (IL=65) sets `Stats.Health.BaseMax` from `ItemValue.MaxUseTimes`, which is `EffectManager.GetValue(DegradationMax = 8, ...)` (`ItemValue::get_MaxUseTimesBase` IL=25). A bicycle is 1500, a minibike 2000, a gyrocopter 3500, a motorcycle 4000 and a truck 8000 - zdtd used 200/250/300, so every vehicle was 10-25x too fragile and its health rode `NetPackageVehicleDataSync` that way. Both tables now resolve it once at load. The same pass stops a vehicle with no `fuelTank` block (stock's bicycle) from accepting fuel: `tank_capacity == 0` on a known kind means no tank, where the refuel path fell back to the rules floor of 100 and consumed the player's gas for nothing.

- **Moving a schematic or an empty jar no longer counts as eating.** XML load marked any item whose name starts with `food` or `drink` eatable. Stock's eatability is the presence of an `ItemActionEat`, and four real rows were false positives - the `foodCanShamSchematic` unlock, the `foodRawMeatBundle` open-bundle action and the empty jars (`drinkJarEmpty` collects water, `drinkJarGrey` has no action) - so moving one on the C2S inventory path read as a meal and restored hunger or water. The heuristic is gone; the builtin/offline fallback stays.

- **`lootcontainer` reads stock's `unique_items`.** The loader looked for the singular `unique_item`, which stock never parses (`LootFromXml` IL_01B8/IL_01CB read `unique_items` into `LootContainer::UniqueItems`); it now honours the plural and keeps the singular as a fallback for the one shipped row that carries it. The row is `count="1"`, so no stock container's contents change - the dedupe would simply have been inert where a modlet relies on it.

- **A player's own character profile reaches the wire.** Stock's `NetPackageRequestToSpawnPlayer` body is `chunkViewDim:i16 | PlayerProfile | nearEntityId:i32` (`PlayerProfile::Read` IL=59), and `GameManager::RequestToSpawnPlayer` stores that profile on the EntityCreationData it writes back - both in the `PlayerId` player-file the owner receives and in the `EntitySpawn` every nearby client gets. zdtd read the dim and discarded the rest, then sent a fabricated `BaseMale`/`White`/variant 1/`Blue01` in the owner's file and `profile = null` to observers, so every player in the world looked like the same default character. The profile is now parsed out of the request, held in fixed storage on the client, and used by both writers: the ECD class follows `isMale` (male/female), the ECD name is the player's login name instead of the literal `"Player"`, and the appearance fields are the client's own. A body with no parseable profile keeps the previous default rather than failing the spawn.

- **The blood moon freezes the weighted party level, not the highest member.** `AIDirectorBloodMoonParty::InitParty` IL_0006 builds its ladder spawner from `partySpawner.CalcPartyLevel()` - `GameStageDefinition::CalcPartyLevel` over the party members, which weights each lower member with `DiminishingReturns` (0.5). zdtd froze `partyHighestGameStage` (the group maximum), so a two-player party at stage 40/80 resolved its nightly waves from the 80 ladder instead of the weighted ~80/52 split stock uses; the count and mix of every `NetPackageSpawnEntity` that night followed. The Game now pushes the weighted level next to the high-water mark and the freeze takes it, falling back to `party_stage` when nothing is pushed (offline tables). A solo player is unaffected: the weighted level of one member is that member's stage.

- **An item passive row's `tier=` anchors are parsed, and an anchor-less row stops inventing a quality ramp.** Stock fills a passive row's anchor array from the first non-empty of `level=`, `tier=` or `duration=` (`PassiveEffect::ParsePassiveEffect` IL_01D3 / IL_026C / IL_0304, each branch jumping past the attributes after it). zdtd read `level=` then let `duration=` override it unconditionally - so it never saw `tier=`, the 633 items.xml rows that anchor a value curve to the item's quality tiers, and would have mis-read a modlet row carrying both. Anchors now come from a shared `parseAnchors` used by both the buff and progression loaders. The same change stops the quality axis from spreading an anchor-less value list over quality 1..6: stock's `_levels == null` branch applies one value flat and rolls two (ModValue IL_03C4 / IL_0427), so a two-value row now projects to its mean instead of ramping across the tier range.

- **A two-anchor, one-value passive row holds its value instead of fading to zero.** Stock pairs a passive row's `level=` and `value=` lists by shape (`PassiveEffect::ModValue` IL_0016), so `level="2,3" value="8"` applies 8 flat across levels 2-3 (IL_0348) and only `level="1,5" value="2,10"` interpolates. zdtd padded the value list to the anchor count before evaluating, which turned every such row into a ramp from the declared value to 0 - 305 stock rows are this shape, and two of them are live: `perkMasterChef`'s `LootProb level="2,3" value="8"` folded 0 from level 3 up (no seeds in dumpster loot) and `resourceGlue`'s `CraftingIngredientCount level="1,5" value="-1"` faded out after tier 1. The evaluator now keeps the value list at its own length and implements stock's other pairing too (one anchor with two values, stock's per-item roll, which the deterministic sim projects as the mean), scans brackets from the highest as stock does, and applies nothing for any other mismatch.

- **A craft ingredient's modified count rounds down and can be free.** `Recipe::CanCraft` (IL=128) derives each ingredient's count through `EffectManager.GetValue(198, ...)`, whose IL_03BC folds the base ops into one accumulator and the percent ops into another and multiplies **once**, then truncates with the caller's `conv.i4`; a result of `<= 0` skips the ingredient entirely (IL_0083) instead of clamping it to 1. zdtd folded the operations one after another, rounded up, and floored the result at 1, so a recipe like `armorPrimitiveHelmet` - which declares `count="0"` cloth and duct tape plus tier rows that add to them - required one of each at tiers below the row's level and demanded `ceil(3 x 1.25) = 4` dart iron where stock charges 3. `ingredientCount` now reproduces stock's two accumulators, truncates, and reports 0 for "not required", which the craft path skips before it resolves the item.
