...
fn optionValue(self: *Game, v: *const packages.GameStatsValues, name: []const u8, buf: []u8) ?[]const u8 {
    ...
    if (std.mem.eql(u8, name, "BloodMoonFrequency")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.clock.bloodmoon_frequency}) catch null;
    ...
    if (std.mem.eql(u8, name, "BloodMoonRange")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.clock.bloodmoon_range}) catch null;
    if (std.mem.eql(u8, name, "ZombieMove")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.zombie_move_day}) catch null;
    if (std.mem.eql(u8, name, "ZombieMoveNight")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.zombie_move_night}) catch null;
    if (std.mem.eql(u8, name, "ZombieFeralMove")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.zombie_move_feral}) catch null;
    if (std.mem.eql(u8, name, "ZombieBMMove")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.zombie_move_bm}) catch null;
    ...
    if (std.mem.eql(u8, name, "BlockDamageAI")) return std.fmt.bufPrint(buf, "{d}", .{stats.block_damage_ai}) catch null;
    if (std.mem.eql(u8, name, "BlockDamageAIBM")) return std.fmt.bufPrint(buf, "{d}", .{stats.block_damage_ai_bm}) catch null;
    if (std.mem.eql(u8, name, "MaxSpawnedAnimals")) return std.fmt.bufPrint(buf, "{d}", .{self.sim.director.max_alive_animals}) catch null;
    if (std.mem.eql(u8, name, "AirDropFrequency")) return std.fmt.bufPrint(buf, "{d}", .{stats.air_drop_frequency}) catch null;
...
pub fn applyGamePrefSet(self: *Game, name: []const u8, value: []const u8) bool {
    ...
    if (std.mem.eql(u8, name, "GameDifficulty")) {
        ...
        // Stock modes 0..4 (4 = delete all); matches serverconfig clamp in config.zig.
        self.drop_on_death = @intCast(@min(@max(v, 0), 4));
