...
/// `V.3.20.10` = ReleaseType V, Major 3, Minor 0x14, Build 0xA; changelog-3.2.0
/// §8: b9->b10). The client's `TryParseSerializedString` requires `Split('.')`
/// to yield exactly 4 fields, so the spaced display form above trips a parse
/// warning. The login package's versionLong stays the display form (`V 3.2.0`,
/// protocol.md VersionLongString packing).
pub const stock_wire_gsi_version = "V.3.20.10";
