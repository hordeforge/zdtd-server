...
// every request.
//
// Build (zig): see mods/BUILDING.md. Committed as zdtd_craftgate.wasm.
...
export fn on_enable() void {
    out.reset();
    out.put("zdtd_craftgate v1.0 enabled (deny recipes named forbidden_*)");
    out.logLine(1);
}

export fn on_shutdown() void {
    out.reset();
    out.put("zdtd_craftgate shutdown");
    out.logLine(1);
}
