...
//
// Build (zig): see mods/BUILDING.md. Committed as zdtd_damagegate.wasm.
...
export fn on_enable() void {
    out.reset();
    out.put("zdtd_damagegate v1.0 enabled (0.5x incoming player damage)");
    out.logLine(0);
}

export fn on_shutdown() void {
    out.reset();
    out.put("zdtd_damagegate shutdown");
    out.logLine(0);
}
