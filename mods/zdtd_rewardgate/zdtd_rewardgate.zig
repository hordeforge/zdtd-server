...
//     <0  withhold the payout; >0 pay that percent of items/exp (150 = 1.5x)
//
// Default policy: 150 (1.5x rewards on every quest). Specialize per quest_def
// for per-quest rules.
//
// Build (zig): see mods/BUILDING.md. Committed as zdtd_rewardgate.wasm.
...
export fn on_enable() void {
    out.reset();
    out.put("zdtd_rewardgate v1.0 enabled (1.5x quest rewards)");
    out.logLine(0);
}

export fn on_shutdown() void {
    out.reset();
    out.put("zdtd_rewardgate shutdown");
    out.logLine(0);
}
