...
//
// Build (zig, committed as mods/zdtd_announce/zdtd_announce.wasm):
//   zig build-exe -OReleaseSmall -target wasm32-freestanding -rdynamic \
//     --name zdtd_announce -femit-bin=mods/zdtd_announce/zdtd_announce.wasm \
//     mods/zdtd_announce/main.zig
...
export fn on_enable() void {
    out.reset();
    out.put("zdtd_announce v2.0 enabled (day + blood-moon announcements)");
    out.logLine(0);
}

export fn on_shutdown() void {
    out.reset();
    out.put("zdtd_announce shutdown");
    out.logLine(0);
}
