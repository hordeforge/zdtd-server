...
**Branch:** `main` at `b0e2565`; working tree clean
**Toolchain:** Zig 0.16, `zig build` + `bash scripts/lint-architecture.sh` (clean), `zig build test` **991/991** (consecutive runs green; flakes fixed, see below)
...
- functionality wave (wire parity, all RE-grounded + scenario-covered):
  ...
  thrower credit) (5c0cdda), NetPackageWorldSpawnPoints on death (b00bcdc)
  + bedroll respawn: placement records the bed, the death list shows it,
  respawn uses it (d155ad5) - note: the sibling swept uncommitted changes
...
All work is committed through `b0e2565`; nothing staged or untracked except
this handoff note.
