...
**Branch:** `main`
**Toolchain:** Zig 0.16, `zig build` + `bash scripts/lint-architecture.sh` (clean), `zig build test` passes (1493 passed / 1 skipped, measured 2026-08-30), `zig build fuzz` green, `make release-check` ok.
...
- ~~**RE-gated difficulty legs**~~ **RESOLVED 2026-08-29 by RE** (combat-damage.md difficultyModifier pin + the sandbox preset extraction): there is no server-side leg to wire. `EntityIncomingDamage` (option 42) never appears in a difficulty code (1.0 on every tier, sandbox_presets goldens) and the stock server never re-applies it: `NetPackageDamageEntity.ProcessPackage` stores the client's claimed strength verbatim and the client applies the modifier in its own `ItemActionAttack.Hit`, so re-scaling C2S claims would double-apply. The AI->player `IncomingDamage` ladder (option 17, 0.5..2.5) is already wired server-side (config applySandboxCode -> Director.sandbox_incoming -> damageScale at the deferred-damage choke, GAP difficulty row WORKS).
