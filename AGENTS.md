...
29. **Wasm-first for behavioral add-ons (ADR 0020).** Anything that is discretionary behavior or server policy — bots, chat commands/filters, announcements and kill-feeds, event observers, custom verdicts — ships as a Wasm plugin by default. Prove the native exception: the sim core (wire encode/decode, LiteNet, interest/replication, chunk stream, ECS authority, persistence, config loading, APM instrumentation) stays native Zig because it is the server itself and runs on the 20 TPS hot path; a plugin cannot and must not touch wire or sim memory directly (sense/queue/query are the only boundary). When adding a feature, default to a plugin; document why anything lands natively.
