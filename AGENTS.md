...
| Bot brains = **Wasm plugins only** (ADR 0026): target selection, aim, movement and combat decisions live in `mods/fps_bot` (the `.wasm` guest); the host `BotManager` is a servant (spawn/replicate/move/LOS gate/sense fill/`bot` verbs) and must not grow native decision logic | Native bot AI in Zig |
