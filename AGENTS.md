...
31. **Core plugins are Zig.** Every plugin under `mods/` ships a Zig source (`mods/<name>/<name>.zig` + `main.zig` wrapper, shared `mods/plugin_common.zig`) rebuilt by `scripts/build-plugins.sh`; the committed `.wasm` is its build output. The only C plugin is `zdtd_bot` (ADR 0026, performance-sensitive brain); `example_chat_filter` is a C example, not core. New core plugins follow the same layout and the naming/manifest rules in `docs/PLUGIN_STANDARDS.md`.
