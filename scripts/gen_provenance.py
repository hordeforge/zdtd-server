...
/* Standalone provenance dashboard, regenerated by scripts/gen_provenance.py.
   ...
   wins on conflict. */
/* Paper cockpit tokens (DESIGN.md): same signal green / warm paper as the
   operator console. No second accent; dark surfaces stay off this page. */
:root{{color-scheme:light;--bg:#f7f5f0;--card:#fffdf8;--sunken:#efece4;--line:#e2ddd0;--edge:#5f686f;--fg:#1a1d21;--muted:#4d545c;--acc:#0f5c37;--acc-ink:#0c4a2d;--ok:#0f5c37;--ok-soft:#dcefe1;--warn:#6b4a00;--warn-soft:#f2dbaa;--err:#8a2318;--err-soft:#f5c9c2;--on-acc:#fff;--shadow:0 1px 2px rgba(26,29,33,.06),0 8px 24px -12px rgba(26,29,33,.18);--sans:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;--mono:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;--fs-micro:0.8125rem;--fs-sm:0.875rem;--fs-lead:1.125rem;--fs-title:1.25rem;--r:10px;--r-chip:6px}}
*{{box-sizing:border-box}}body{{font-family:var(--sans);margin:0;background:var(--bg);color:var(--fg);line-height:1.45;-webkit-font-smoothing:antialiased}}
:focus-visible{{outline:2px solid var(--acc);outline-offset:2px}}
header{{padding:1rem 1.25rem;border-bottom:1px solid var(--line);background:var(--card)}}
header h1{{font-size:var(--fs-title);margin:0;font-weight:700;letter-spacing:-0.01em}}header .meta{{color:var(--muted);font-family:var(--mono);font-size:var(--fs-micro)}}
main{{padding:1rem 1.25rem;display:grid;gap:1rem;max-width:64rem;width:100%;margin-inline:auto}}
section{{background:var(--card);border-radius:var(--r);padding:0.85rem 1rem;border:1px solid var(--line);box-shadow:var(--shadow)}}
section h2{{font-size:var(--fs-sm);color:var(--acc-ink);text-transform:uppercase;letter-spacing:0.06em;font-weight:600;margin:0 0 0.6rem}}
...
.mapcat{{display:flex;flex-direction:column;width:100%;text-align:left;background:var(--sunken);border:1px solid var(--line);border-radius:var(--r-chip);padding:0.5rem 0.6rem;cursor:pointer;color:inherit;font:inherit}}
...
.mapcat:not(.clean){{border-color:#cbab72}}
...
.badge{{font-family:var(--mono);font-size:var(--fs-micro);color:var(--warn);background:var(--warn-soft);border:1px solid #cbab72;border-radius:999px;padding:0.05rem 0.45rem;white-space:nowrap}}
...
.cell.PARTIAL{{background:var(--warn-soft);color:var(--warn)}}
.cell.MISSING{{background:var(--err-soft);color:var(--err)}}
...
.toolbar input[type=search]{{flex:1;min-width:12rem;background:var(--card);border:1.5px solid var(--edge);color:var(--fg);border-radius:var(--r-chip);padding:0.4rem 0.6rem;font:inherit}}
.toolbar input[type=search]:focus{{outline:2px solid var(--acc);outline-offset:1px;border-color:var(--acc)}}
.chips{{display:flex;gap:0.35rem;flex-wrap:wrap}}
.chip{{background:var(--card);border:1px solid var(--edge);color:var(--muted);border-radius:999px;padding:0.2rem 0.7rem;font-size:var(--fs-micro);cursor:pointer;font-family:var(--mono)}}
.chip[aria-pressed=true]{{border-color:transparent}}
.chip[aria-pressed=true].chip-w{{background:var(--ok);color:var(--on-acc)}}.chip[aria-pressed=true].chip-p{{background:var(--warn-soft);color:var(--warn);border:1px solid #cbab72}}.chip[aria-pressed=true].chip-m{{background:var(--err-soft);color:var(--err);border:1px solid #d59f96}}.chip[aria-pressed=true].chip-a{{background:var(--acc);color:var(--on-acc)}}
.cat{{cursor:pointer}}
.cat:hover,.cat:focus-visible{{background:var(--sunken)}}
.cat.open th{{color:var(--acc-ink)}}
.cat.open::after{{content:"▾";color:var(--acc-ink);padding-left:0.4rem}}
.cat:not(.open)::after{{content:"▸";color:var(--muted);padding-left:0.4rem}}
.feat-row td{{background:var(--sunken);padding:0.4rem 0.6rem}}
.feat-wrap{{max-height:22rem;overflow:auto;border:1px solid var(--line);border-radius:var(--r-chip)}}
...
.st.WORKS{{color:var(--ok)}}.st.PARTIAL{{color:var(--warn)}}.st.MISSING{{color:var(--err)}}.st.ad-hoc{{color:var(--acc-ink)}}
...
footer{{padding:0.75rem 1.25rem;color:var(--muted);font-size:var(--fs-micro);border-top:1px solid var(--line);background:var(--card)}}
@media(max-width:36rem){{main,header,footer{{padding-left:0.75rem;padding-right:0.75rem}}section{{padding:0.75rem}}}}
@media(forced-colors:active){{:root{{--bg:Canvas;--card:Canvas;--sunken:Canvas;--line:CanvasText;--edge:ButtonText;--fg:CanvasText;--muted:GrayText;--acc:LinkText;--acc-ink:LinkText;--ok:CanvasText;--ok-soft:Canvas;--warn:Highlight;--warn-soft:Canvas;--err:MarkText;--err-soft:Canvas;--on-acc:Canvas}}body,section,header,footer{{background:Canvas;color:CanvasText;border-color:CanvasText;box-shadow:none}}:focus-visible{{outline:3px solid Highlight}}}}
</style></head>
<body>
