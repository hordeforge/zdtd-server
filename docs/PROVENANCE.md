...
| `packages.zig` | 155 | partial: audited by `--lines` section, since no small filter set covers the whole file. Lines 480-3400 verified (108 pairs, 0 survivors after the fixes): the PlayerId join body, PlayerSpawnedInWorld, lastSpawnPosition, the three motion bodies, POI metadata, the chunk envelope, DamageEntity, EntityStatChanged, the inv transaction body, EntityCollect, the lock responses, the full GameStats blob. 47 pairs above line 3400 unaudited |
...
| `src/protocol.zig` | R | Wire constants from ../../7dtd-engine-research/docs/protocol.md (V3.2.0 pin; V3.1.0/V3.0.1-era goldens still cited). Package IDs are dynamic (PackageIds map); never hard- |
...
| `src/world/sky.zig` | R | Stock SkyManager day/night model (RE `../7dtd-engine-research/docs/entity-ai.md` SkyManager pin 2026-08-26): TimeOfDay, UpdateSunMoonAngles sun target, CalcDayPercent curve, GetLightLevel ambient term. Slice 1 of the clone-side world-light model; the moon term SHIPS 2026-08-27 (moonAmbientScale in step.zig), block light / moving lights / shade stay recorded later slices |
