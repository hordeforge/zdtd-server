...
| `items.zig` DegradationPerUse / TargetArmor | flat per item | A | items.xml DegradationPerUse (base_set, per-use durability wear; wired into degradeUse) and untagged TargetArmor (perc_add, armor penetration; wired into armorMitigationVs at the damage chokes, RE GetTotalPhysicalArmorRating IL=47). Perk-tag-gated TargetArmor rows and BlockDamage/StaminaLoss/HarvestCount/LootProb recorded, not guessed |
