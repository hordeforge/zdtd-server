...
| `packages.zig` | 155 | not run. Its 86 tests share no small set of name substrings, so a filter wide enough to cover the file selects nearly the whole suite and the speedup disappears: a 30-mutant run managed 2 in 20 minutes. Auditing it needs either an unfiltered run (~10 h) or per-builder filters chosen one section at a time |
