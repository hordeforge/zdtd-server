...
| `packages.zig` | 155 | partial: audited by `--lines` section, since no small filter set covers the whole file. Lines 480-1700 verified (59 pairs, 0 survivors after the fixes): the PlayerId join body, PlayerSpawnedInWorld, lastSpawnPosition, the three motion bodies, POI metadata, the chunk envelope, DamageEntity. Lines 1701-2500 have their gaps closed but no clean re-run yet. 96 pairs above 2500 unaudited |
