...
| `src/assets/modlets.zig` | R | Stock `ModManager` subset for XML-only modlets (../7dtd-engine-research/docs/admin/mod-loading.md §1-2): Mods/ root scan, ModInfo.xml V2 parse (Name/DisplayName/Version rules), Config dir collection in mod order, Bundles/ tolerance (never read, PRD R11), code-mod warning (DLLs never hosted). PRD 0003 / RFC 0003  Each mod's `Version` is kept and `isLoaded`/`versionByName` answer `<conditional>` tests (2026-09-13).|
