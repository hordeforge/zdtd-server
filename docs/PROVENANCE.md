...
| `server/game/init_world.zig` ambient seeds | 11 entities at join (`listents`): 6 POI traders + 3 seed zombies/sleeper + 1 animal + 1 vehicle + 1 turret | stock spawns lazily: `listents` = player only (1) at join, zombies later via AIDirector near players, traders on POI load | P3 | known divergence from SUT join-probe 2026-08-12 (7dtd-loadgen compare-sut); not in live audit |
| `ecs/aidirector.zig` WorldClock rate | flat 0.41-0.44 game-min/s (DayNightLength 60 -> `seconds_per_hour` 150) | stock measured 0.33-0.35 game-min/s, Day 1 07:00-07:07 (DayNightLength 60 live stat; clock scaling differs from flat) | P2 | known divergence from SUT join-probe 2026-08-12 (7dtd-loadgen compare-sut, 2 runs); not in live audit |
