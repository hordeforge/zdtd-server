...
| `assets/loot.zig` loot-entry `<requirement>` children | gated entries are flagged (`LootEntry.gated`) and omitted from every roll path; ungated entries roll as before | `loot.xml` `LootEntryRequirement*` leaves (63 `RandomRoll`, 14 `Progression`, 5 `Biome`, 3 `SandboxOption`, 1 `QuestTags`, 1 `CVar`; RE loot-economy.md:569 for `LootEntryRequirementRandomRoll`: `LeftSide = Lerp(minMax.x, minMax.y, RandomFloat)`, `RightSide = GetFloatValue(player, value, 0)`) | P2 | **Fixed (omit) 2026-09-12**: the loader ignored the child element, so a gated entry rolled at its base prob - `groupWorkingStiffsBooks` (no `prob`, i.e. 1.0, gated on `RandomRoll @$perkBookwormChance`) filled every Working Stiffs crate. Omitted now (missing beats fake; stock's gate refuses for a player without the perk). Residual: the evaluator (cvars/levels/biome/sandbox) and the `onSelfProgressionUpdate` row that writes `$perkBookwormChance` |
