...
| `src/assets/modlets.zig` | R | Stock `ModManager` subset for XML-only modlets (../7dtd-engine-research/docs/admin/mod-loading.md §1-2): Mods/ root scan, ModInfo.xml V2 parse (Name/DisplayName/Version rules), Config dir collection in mod order, Bundles/ tolerance (never read, PRD R11), code-mod warning (DLLs never hosted). PRD 0003 / RFC 0003 |
...
| `src/assets/progression.zig` | A | progression.xml: level curve + attribute/perk/book catalog (names, max levels, costs) + the passive_effect rows with the `<requirement>` gates stock applies to each (group-level direct children plus the row's own, MinEffectGroup::ParseXml IL=103 / PassiveEffect::ParsePassiveEffect IL=423). `<book>` blocks join the catalog because a book is a progression value items.xml grants via SetProgressionLevel(-1). Requirement evaluation is `src/assets/requirements.zig` (incl. HoldingItemHasTags IL=37 against the held item's Tags, SandboxOptionBool IL=18 against the decoded SandboxCode, and ArmorGroupLowestQuality IL=34 against the worn armor groups' lowest quality). `<level_requirements>` blocks (301 stock: 250 ProgressionLevel + 51 PlayerLevel), the per-level `override_cost` tables (7 stock rows) and the `<perks>` container cost/max defaults parse into the catalog for the purchase gate (`Table.calculatedMaxLevel`, ProgressionClass::GetCalculatedMaxLevel IL=343) |
...
| `assets/loot.zig` loot-entry `<requirement>` children | gated entries carry `LootEntry.gate`: `Biome` resolves at roll time against the container's biome (`LootGateCtx.biome_name`, from `biome_layers.nameById`), the player-state classes (`RandomRoll`, `Progression`, `CVar`, `SandboxOption`, `QuestTags`) are omitted from every roll path, ungated entries roll as before | `loot.xml` `LootEntryRequirement*` leaves (63 `RandomRoll`, 14 `Progression`, 5 `Biome`, 3 `SandboxOption`, 1 `QuestTags`, 1 `CVar`; RE loot-economy.md:569 for `LootEntryRequirementRandomRoll`: `LeftSide = Lerp(minMax.x, minMax.y, RandomFloat)`, `RightSide = GetFloatValue(player, value, 0)`) | P2 | **Fixed (omit) 2026-09-12**: the loader ignored the child element, so a gated entry rolled at its base prob - `groupWorkingStiffsBooks` (no `prob`, i.e. 1.0, gated on `RandomRoll @$perkBookwormChance`) filled every Working Stiffs crate. Omitted now (missing beats fake; stock's gate refuses for a player without the perk). **Biome gates resolve since 2026-09-12** (the container fill passes its own biome: `groupShamwaySafe`'s wasteland-only `plantedGraceCorn1Schematic` rolls in the wasteland and not elsewhere, with no ctx it stays omitted). Residual: the player-state evaluator (cvars/levels/sandbox/quest tags) and the `onSelfProgressionUpdate` row that writes `$perkBookwormChance` |
...
| `server/game/init_world.zig` ambient seeds | the near-spawn demo hostiles (2 zombies + sleeper + animal) are seeded at world init by default and switchable with `[sim] starter_zombies = false` | stock spawns lazily: `listents` = players only (1-3) at join, animals/zombies later via AIDirector near players, traders on POI load | P3 | **Configurable 2026-09-12**: the demo seeds are now a documented, switchable divergence (docs/DIVERGENCES.md 6.2) with the stock-lazy setting one key away; first recorded from the SUT join-probe 2026-08-12 (7dtd-loadgen compare-sut) |
...
| `server/game/chunk_fill.zig` container loot timing | the initial roll happens at chunk/prefab TE load with `partyLootStage()`; `touched` rides the first `setSlot` and `touched_day = clock.day`; only "container has loot" state survives | stock rolls a placed container **on first open** (`LootManager.LootContainerOpened`: return if `bTouched`; set `bTouched`, stamp `worldTimeTouched = world.GetWorldTime()`, resolve the `LootContainer` by `lootListName`, roll **only if the container is empty**, fire MinEvent 101/100 on the opening player) with the **opener's** loot stage (`LootContainer.Spawn` -> `RandomSpawnCount(min,max,abundance)` clamped to slots -> `SpawnLootItemsFromList`); loot-economy.md "Generation gate"/"The roll" | P2 | **Open (recorded 2026-09-12)**: contents are fixed before anyone sees the chest, the stage is the party's at load (a low-level player's presence lowers a high-level player's chest), and the `LootEntryRequirement` player-state gates can never see the opener, so those rows stay `EntryGate.other`. Sketch: derive `slot_count` from loot.xml at load without rolling, roll in the `InventoryDataRequest` open path with `lootStageForPlayer(peer)` (the bio ctx already reaches `fillContainerFromLoot`), set `bTouched` + `worldTimeTouched` even on an empty roll (stock sets them before the roll), and keep the respawn re-roll on the same path |
| C2S payload decode (litenet/net.zig) | every C2S payload decodes on the current build: **0** `net_payload_errors` / `decode_rejects` across the pregen matrix | stock joins all pregen worlds cleanly (0 overflows) | - | **Resolved 2026-09-12 (matrix re-run)**: the 2026-08-12 report (`payload failed error=Overflow n=1` per join, join FAILS on Pregen06k01) no longer reproduces. Pregen06k01: 2 wander bots x 11 lives, 1200 ticks, `join_fail=0`, `net_payload_errors=0`, `decode_rejects=0`, every enter PASS. Pregen08k02: 3 bots, 25 PASS joins, same zeros. The Pregen06k01 join failure the row recorded was the S2C `WorldSpawnPoints` 512-byte builder overflow, fixed 2026-08-29 (buffer 1024 + regression test); the decode path the row cites is still instrumented (`server/game/net.zig` `logPayloadErr`) and stays at 0. The residual in both runs is a transient `stream_errors` WindowFull on the chunk stream, which leaves the unsent chunk pending and retries it next period rather than dropping it |
