...
| `src/assets/buffs.zig` | A | buffs.xml: metadata + passive_effect rows, plus the bounded passive-effects VM (`trackedDeltasAt`/`effectTotals`, revertible by recompute-from-set, requirement- and tag-gated) and the triggered-effect engine (`evaluateTriggered`). Full triggered_effect action set is later; `tagsMatch` is PassiveEffect::hasMatchingTag IL=53 with the stock ctor defaults; passives carry their effect_group + nested row requirements (`scanEffectGroup`) |
