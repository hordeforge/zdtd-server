...
| `packages.zig` | 155 | partial: audited by `--lines` section, since no small filter set covers the whole file. Lines 480-2500 verified (67 pairs, 0 survivors after the fixes): the PlayerId join body, PlayerSpawnedInWorld, lastSpawnPosition, the three motion bodies, POI metadata, the chunk envelope, DamageEntity, EntityStatChanged, the inv transaction body. 88 pairs above line 2500 unaudited |
