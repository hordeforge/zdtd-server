...
| `max_claimed_explosion_radius` (`server/c2s/blocks.zig`) | 6.0 | R | Cap on C2S-claimed explosion radii (block + entity): largest stock ExplosionData.EntityRadius is 6 (entities.xml `explosion` on cop/feral: radius_blocks 5, radius_entities 6); a forged blob must not carve the whole map or damage every loaded entity (2026-08-27) |
