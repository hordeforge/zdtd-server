...
| `src/assets/maxdamage.zig` | A | MaxDamage lookup: blocks.xml name→hp + optional AssignIds map from .blocks.nim or a version-matched id\tname dump. Without a full id map, callers fall  `MaxDamage` / `Material` / `Stage2Health` / power / turret props resolve through `Extends` with each child's `param1` exclusion list, and so does the `Shape`-derived `Has45DegreeRotations` fact (only `DistantDecoTree` sets it, `BlockShapeDistantDecoTree` IL=6); `explosionresistance`, material `StabilitySupport` (block default), `collidable`, `movement_factor`, `lightopacity` and `CanDestroy` parse from materials.xml (2026-09-13, CanDestroy 2026-09-14: only Mbedrock declares false, and the damage paths fail closed on `canDestroyFor` at the `addBlockDamage` choke point and the C2S break decision).|
...
| `src/assets/gameevents.zig` | A | gameevents.xml `<action_sequence>` loader for the server-executable subset (ModifyEntityStat, RemoveDeathBuffs, AddBuff + its CVar gate, ModifyCVar, AddXPDeficit, RemoveItems, AddStartingItems), plus `isClientAction` classifying each leg by its C# base (`ActionBaseClientAction` descendants run on the client, `ActionBaseTargetAction` on the server). A sequence holding an action class, a non-`Set`/`SetMax` stat operation, or a `<decision>`/sequence-level `<requirement>` this server does not evaluate is marked unsupported and refused whole (`missing beats fake`) |
| `src/assets/blockplaceholders.zig` | A | blockplaceholders.xml `<placeholder>` lists (466 stock placeholders) with their weighted `<block>` targets, parsed once: target names resolve to runtime block ids, the `sandboxoption` gate is evaluated against the decoded server code (the value is fixed per run), the target's `Has45DegreeRotations` shape fact is passed in and decides the rotation band, and the per-cell pick (biome filter, `RandomFromSeedOnPos` seeding, weighted walk, `randomrotation`) is left to the prefab paint pass, which has the world position and the biome |
| `src/assets/localization.zig` | A | `Config/Localization.csv` merge for the joining-client localization patch blob. The base file's header row is the column index (stock's `languageToColumnIndex`) and each modlet's `Config/Localization.csv` contributes cells in mod order, a later mod winning, exactly the cells stock marks in `patchedCells`. `csvText` is stock's `Localization::WriteCsv` IL=190 shape (`KEY` plus the header names, then one line per patched key with the patched cells quoted when they carry a quote or a comma), and `deflatedCsv` raw-Deflates it the way `DeflateOutputStream(_, 3)` does before `NetPackageLocalization` ships it in 128 KiB parts (`prepareDataPackets` IL=107). Stock reads the CSV only; `.txt` is never read |
| `src/util/game_random.zig` | R | The game's `System.Random`-algorithm generator (`GameRandom`: 56-entry `SeedArray`, `MBIG` 2147483647, `MSEED` 161803398, the `inext`/`inextp` subtractive step, `Sample() = InternalSample() * 4.6566128752458E-10`) plus `Utils::RandomFromSeedOnPos`'s coordinate fold. Goldens in its test come from Mono's `System.Random`, i.e. the same algorithm. Used by the blockplaceholders per-cell roll so the target matches the one stock picks |
...
| `src/assets/xml_patch.zig` | A | Clean-room config XML patches (stock XmlPatcher subset; mod-loading.md §5.3). Full verified op catalog: set/setattribute(/byxpath), remove(/byxpath), removeattribute(/byxpath), append/prepend(/byxpath), insertafter/insertbefore(/byxpath), csvoperations, include (@modfolder: tokens); `<conditional>` evaluates `mod_loaded`/`mod_version` and skips an unevaluated expression with a warning (an unported NCalc function must not stop the boot). Applies modlet Config dirs (mod order) + --config-overrides (file order)  Patch op vocabulary matches stock's registered names (`csv` too); `setattribute` reads `name=`, `set` handles `/@attr` and element ReplaceNodes, append/prepend target attribute text, remove refuses an attribute path, ops apply to every XPath match, predicates cover contains/starts-with/and/or/existence/position, `<conditional>` evaluates `mod_loaded`/`mod_version` (unknown expressions skip with a warning), and a patch resolves as `<mod>/Config/<configName>` (subdirectory configs included) (2026-09-13). Predicates cover element existence with a nested clause (`block[property[@name='Tags' and @value='X']]`, the bracket-aware clause splitter keeps the nested ` and ` inside the group) and `[last()]`; the match collector walks the path segment by segment so every match of `/root/child[...]` is found instead of only the first, which also corrects `[last()]` (2026-09-14). A patch that cannot be applied no longer stops the boot: an unknown op warns and skips the element (stock XmlPatcher IL_0176-0204), a root that is not `<configs>` is a container whose children are ops (stock walks the root's children; 0-SCore ships `<SCore name=...>`), a known op without `xpath` abandons that one file, and `<include filename=...>` resolves against the including file's directory (`XmlPatchMethods::Include`, 2026-09-14). An xpath this subset cannot parse is logged with its count instead of vanishing (2026-09-14). `csv` honours the `delim` attribute (one character or the literal `\n`, default comma) with per-entry add/remove (2026-09-13). |
...
| `src/server/game/game_events.zig` | R | Runs a parsed gameevents.xml sequence for a player (`GameEventManager` death/respawn semantics: the client's HandleActionClient IL=416 only forwards NetPackageGameEventRequest, the server branch IL_006B executes). Owns the `game_on_death_*`/`game_on_respawn_*` name selection by DeathPenalty (PlayerMoveController IL_0E38, EntityPlayer IL=71), stat writes, death-buff removal with the injured tag exclusion, the AddBuff CVar gate and cvar writes, and the client legs: every `ActionBaseClientAction` action is answered with one NetPackageGameEventResponse ClientSequenceAction (12) keyed by stock's `BaseAction.actionKey` from the parsed order (`SetActionKeyData` IL=23, root `<name><index>`), which is the whole server leg for AddXPDeficit/RemoveItems/AddStartingItems. No stock numbers: the effects come from the parsed table |
