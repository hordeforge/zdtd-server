...
| `src/assets/entities.zig` | A | entityclasses.xml loader: name → Unity Mono hash, kind, HP, death loot list, day/night speeds (MoveSpeed/MoveSpeedNight shamble, MoveSpeedAggro "min,max" = day/night chase, entity-ai.md GetMoveSpeed/GetMoveSpeedAggro + the stock XML comment "min/max (like day or night)") and the SightLightThreshold "min,max" pair (stock "-2,150", CanSeeStealth IL=21). Also resolves the inherited AITask-* list into `ai_attack` (timid animals carry no attack task and never pick approach_attack; the attack-task name set is a stock-AI RE constant) |
...
| `Ai.chase_speed` | 2.2 | A | **Floor** (night): `entityclasses.xml` MoveSpeedAggro max ×1.6 when non-zero; the day branch uses MoveSpeedAggro min (`chase_speed_day`), same floor. RE entity-ai.md GetMoveSpeedAggro (dark → passive 134 max, day → passive 133 min; the stock XML comment "min/max (like day or night)" pins the split) |
...
| `ecs/systems.zig` TickServer light consts (`stealth_crouch_light_scale` 0.6, `stealth_light_passive_blend_a` 0.32, `stealth_light_passive_blend_b` 0.68, `stealth_light_level_max` 200, `stealth_self_light_dark` 0.1) | 0.6 / 0.32 / 0.68 / 200 / 0.1 | R | `PlayerStealth.TickServer` IL=432 exact chain (crouch x0.6 IL_00A6; the (0.32 + 0.68 x passive89) x 100 fold IL_0121-013F; FastClamp 0..200 IL_0140-014A; selfLight < 0.1 IL_010A) |
