...
| `src/assets/modlets.zig` | R | Stock `ModManager` subset for XML-only modlets (../7dtd-research/docs/mod-loading.md §1-2): Mods/ root scan, ModInfo.xml V2 parse (Name/DisplayName/Version rules), Config dir collection in mod order, Bundles/ tolerance (never read, PRD R11), code-mod warning (DLLs never hosted). MODLETS_PRD/PLAN |
...
| `src/assets/paths.zig` | A | Resolve Data/Config XML paths, modlet + override patch dirs (base → mods → overrides, MODLETS_PRD R6/R7), generic tryLoad |
