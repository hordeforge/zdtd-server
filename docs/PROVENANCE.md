...
| `server/game/init_world.zig` ambient seeds | the near-spawn demo hostiles (2 zombies + sleeper + animal) are seeded at world init by default and switchable with `[sim] starter_zombies = false`; Trader Jen, the minibike, the seed chest (10 wood / 3 food) and the virtual generator + turret seed **unconditionally** | stock spawns lazily: `listents` = players only (1-3) at join, animals/zombies later via AIDirector near players, traders on POI load | P3 | **Partially configurable 2026-09-12**, scope corrected by the hardcode audit 2026-09-12 (B2): only the hostiles are behind the switch, so `starter_zombies = false` is not yet a stock-lazy fresh world. Open: one `[sim] demo_seed` switch for the trader/minibike/chest/power seeds. First recorded from the SUT join-probe 2026-08-12 (7dtd-loadgen compare-sut) |
