...
| MoveHelper physics / collision | PARTIAL (2026-08-21: collide-and-slide against the block grid (body ~0.35×1.8, axis-separated like the stock CC Move), step-up of 1 block, gravity per the stock formula (World.Gravity 0.08/tick with the 0.98 y-drag, ~1.6 blocks/s²), the stock jump (a fully blocked, grounded AI fires an impulse sized to clear jump_height, rules.ai, stock StartJump heightDiff ~1.3, entity-ai.md 2030-2034; jumpDelay 1 s gate, entity-ai.md 3228), and door-opening: a zombie pressed against a door on its path opens it (SetOpen meta bit + broadcast, RE entity-ai.md CheckForDoorAndOpen) instead of chewing, and the AI solid probe treats an open door as passable (door id detected by the stock door-naming set, blocks.xml). Bots keep their own stepMoveCollide (jump/door are host affordances, deferred). Remaining: dig-through (wander-blocked digging), swimming strokes, elevator, entity push. RE entity-movement.md) |
