...
| Falling blocks | PARTIAL (2026-08-21: the stock default path is per-cell singular `fallingBlock` entities - group mode EntityFallingBlocks.Enabled is false by default - so stabilityAfterSetBlock now spawns one singular entity per collapsed cell whose block `ShowModelOnFall` resolves true (blocks.xml property, default true per Block.il.txt 1876-18A2; 13 explicit-true / 67 explicit-false in stock), each at its cell center with the stock -0.1..0.1 Y offset and a deterministic position-seeded horizontal impulse; full BlockValue rawData (rotation/meta) rides the ECD; the replicate path branches class fallingBlock (n=1) vs fallingBlocks (n>1). The opt-in group mode and its BFS grouping (GroupBounds cap) are not implemented; remaining: Fall-event item drops on death (itemsToDrop[Fall] prob 1), landing damage (massKg*vy*-0.05 cap 40, passive 164), impact particles/audio, re-place via OptionsStabSpawnBlocksOnGround 148 (default off), group-size IL pin. RE entity-ai.md LetBlocksFall / EntityFallingBlock.OnUpdateEntity) |
