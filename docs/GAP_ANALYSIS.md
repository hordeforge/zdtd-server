...
| `NetPackageEntityWaypointList` / map markers | SHIPPED (was scored SHIPPED on the 2026-08-26 re-audit for the wrong reason - the client-local receive half; corrected to PARTIAL 2026-09-06 when the server-sender sweep flagged the missing send, then closed the same day. The `ProcessPackage` IL=22 reading is right - it is client-local, `GetPrimaryPlayer` plus the local `WaypointCollection` - but that only describes the receiving half, and the row treated "no C2S role" as "nothing to do". The server *sends* it: `VehicleManager.UpdateVehicleWaypointsForPlayer` (IL=69) walks the vehicle ECD list for `belongsPlayerId` and, when that player is remote, ships the id/pos pairs so their map shows where they parked; `DroneManager.UpdateWaypointsForPlayer` (IL=96) is the same for drones. Closed 2026-09-06: `Vehicle.owner_slot` tracks the placing player's client slot (stock `Entity.belongsPlayerId`, set from `entityThatPlaced` at placement), the stock body (`listType` i16 Vehicle=0, count, `(entityId, Vector3)` pairs) ships from the join bundle via `sendVehicleWaypoints`, and unowned vehicles are nobody's waypoints. Drones have no sim entity, so the drone list type stays unsent - stated with the subsystem, not the package; the day drones exist the same builder carries them) |
