...
- **POI sleeper volumes: parse, trigger, spawn at authored markers** `WORKS` (2026-08-25):
  ...
  wire flag (sleeping pose until the wake flips) all ship. Residuals are
  RE-blocked: `triggered` re-arm (stock respawnTime) and the sleeper pose
  byte table from the marker block name are not in RE (rows 2018 /
  entity-ai); gamestage count scaling beyond the stage group caps is a
  fidelity nicety, recorded.
