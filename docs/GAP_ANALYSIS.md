...
| Package | Priority |
|---|---|
| `NetPackageDamageEntity` full field semantics | PARTIAL (the field-level scoring below is unchanged and still WORKS; downgraded 2026-09-06 because "the builder emits" describes a builder that no send path calls - the server-sender sweep found the name is never passed to `sendGame` or a broadcast, matching the dismember row's "emits no S2C NetPackageDamageEntity at all". Stock does send it: `SendPacketToTrackedPlayersAndTrackedEntity` fans the damage to the victim's trackers so their clients play the hit reaction. 2026-08-26 re-audit, still accurate for the body itself: the builder produces the FULL stock body - all 31 fields in read-IL order (entityId, damageSrc/Typ, strength, hitDirection, hitBodyPart, movementState, pain/fatal/critical, attacker, dirV, blockPos, hitTransform, uvHit, damageMultiplier, random, ignoreConsecutive/buff/dismember/cripple/crawler flags, bonusDamageType, StunType/Duration, bFromBuff, ArmorSlot/Group/Damage, optional ItemValue); parseDamageHead consumes the head and validates the acted-on fields (entityId/source/type/strength/fatal/trapKillXP). Correction 2026-09-06: calling the remaining flags "hit-info cosmetics" understated one of them - see the dismember row below) |
