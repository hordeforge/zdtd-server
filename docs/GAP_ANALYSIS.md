...
| `NetPackageEntityWaypointList` / map markers | PARTIAL (was scored SHIPPED on the 2026-08-26 re-audit; corrected 2026-09-06 when the server-sender sweep flagged it. The `ProcessPackage` IL=22 reading is right - it is client-local, `GetPrimaryPlayer` plus the local `WaypointCollection` - but that only describes the receiving half, and the row treated "no C2S role" as "nothing to do". The server *sends* it: `VehicleManager.UpdateVehicleWaypointsForPlayer` (IL=69) walks the vehicle ECD list for `belongsPlayerId` and, when that player is remote, ships the id/pos pairs so their map shows where they parked; `DroneManager.UpdateWaypointsForPlayer` (IL=96) is the same for drones. zdtd keeps vehicles as ECS entities with no owner field and has no drones, so it sends nothing and a player's parked vehicles do not appear on their map. Closing it needs the owner association first, not a builder) |
