...
| MoveHelper physics / collision | PARTIAL (2026-08-21: collide-and-slide against the block grid (body ~0.35×1.8, axis-separated like the stock CC Move), step-up of 1 block, gravity per the stock formula (World.Gravity 0.08/tick with the 0.98 y-drag, ~1.6 blocks/s²), and the stock jump: a fully blocked, grounded AI fires an impulse sized to clear jump_height (rules.ai, stock MoveHelper StartJump heightDiff ~1.3, entity-ai.md 2030-2034) and sails at the arc apex while rising (jumpDelay 1 s gate, entity-ai.md 3228). Bots keep their own stepMoveCollide (jump is a host affordance, deferred). Remaining: dig-through, swimming strokes, elevator, entity push, door-opening. RE entity-movement.md) |
