...
| Stealth / crouch | PARTIAL (2026-08-20: crouch replicates via NetPackageEntityAliveFlags bit 512 (IsCrouching); a crouched player's hearing gate muffles by crouch_hear_scale (0.5; stock per-clip muffledWhenCrouched from sounds.xml is data-driven) and sleepers only detect crouched players within crouch_sleeper_detect_range (5; stock light-based FastLerp(3,15,light) leg is RE-blocked - no server light channel). Movement-noise volume model wired 2026-08-26: the sounds.xml `<Noise>` table (assets/noise.zig, 1312 rows keyed by SoundDataNode name) + the PlayerStealth fold (systems.systemStealth: NotifyNoise accumulation, CalcVolume curve, sleeperNoiseVolume cap 360 -> volume wake, heat map, attraction heard-test `noiseVolume x (1+feralSense) / (dist x 0.6 + 0.4) x DetectUsScale >= 1`), all `[rules.ai] stealth_*` tunables; stock-dedi emission reality pinned in entity-ai.md: the C2S sound relay is audio-only on a dedicated server (PlaySoundAtPositionServer skips NotifyNoise when IsDedicatedServer; NetPackageEntityStealth server branch ignores client noise levels), so the model's inputs are server-side sounds only (explosions/mines/animals/minibike via Audio.Manager.SignalAI / GameManager.explode) - none player-attributed in the sim yet, the ring stays ready as those sources land. Missing (recorded): the per-position world-light legs of the CanSeeStealth model (block light, moving lights, moon/shade - slice 1 day/night ambient ships 2026-08-26 via world/sky.zig driving the stealth S2C light byte; the sim sight/sleeper light legs consume it once block light lands), smoke/smell stealth) |
