...
| `NetPackageEntityStatChanged` / stats / buffs | PARTIAL (join sends Health/Stamina/Food/Water stock body; player Health replicates from the tick pass on `dirty.hp` per `EntityStats::TickWait` (asm.il:199393); buff set is server-owned via AddRemoveBuff with join sync + own-buff rejoin bundle; stealth meter S2C wired 2026-08-26 - NetPackageEntityStealth (id:i32, data:u16: noise 7-bit <<8, alert<<15, crouch bit 0) every 16 ticks on change, stock PlayerStealth.TickServer IL_0470, light = slice-1 day/night ambient (2026-08-26, world/sky.zig; block-light/moon/shade slices recorded); NPC stat-change stays deferred - traders are invulnerable, no stat they change) |
...
| Line of sight / hearing / smell | WORKS (2026-08-20: sense gate ships - per-class view cone (entityclasses MaxViewAngle, stock cctor default 180 halved like IsInFrontOfMe), block-LOS sight via Voxel.Raycast-equivalent, hearing within hear_range that passes walls, and a smell radius that passes walls with a bleeding-player extension (buffInjuryBleeding → cSmellRadiusBleed 25, else cSmellRadiusMin 10), RE entity-ai.md CanEntityBeSeen + PlayerStealth + EntityAlive/EntityClass cctor defaults. Sub-note 2026-08-26: CanSeeStealth's light-level leg IS server-evaluated (PlayerStealth.TickServer IL=432 writes lightLevel from LightManager.GetStealthLightLevel; EAITarget.check reads it) - the model is pinned in entity-ai.md; the clone-side world-light leg ships slice 1 (2026-08-26, world/sky.zig: TimeOfDay/UpdateSunMoonAngles/CalcDayPercent + the GetLightLevel ambient term feeding the stealth S2C light byte; AmbientTotal collapsed to the day curve); block light (chunk light), moving lights, shade and the moon term stay recorded slices) |
| Stealth / crouch | PARTIAL (2026-08-20: crouch replicates via NetPackageEntityAliveFlags bit 512 (IsCrouching); a crouched player's hearing gate muffles by crouch_hear_scale (0.5; stock per-clip muffledWhenCrouched from sounds.xml is data-driven) and sleepers only detect crouched players within the stock FastLerp(3,15,lightAttackPercent) range (2026-08-26: the light leg is no longer RE-blocked - the slice-1 day/night ambient (world/sky.zig) feeds lightAttackPercent (0.89 passive in deep dark else 1) and the sleeper wake scan, `[rules.ai] crouch_sleeper_detect_min/max` + `stealth_light_passive`; the old flat 5-m crouch floor is gone). Movement-noise volume model wired 2026-08-26: the sounds.xml `<Noise>` table (assets/noise.zig, 1312 rows keyed by SoundDataNode name) + the PlayerStealth fold (systems.systemStealth: NotifyNoise accumulation, CalcVolume curve, sleeperNoiseVolume cap 360 -> volume wake, heat map, attraction heard-test `noiseVolume x (1+feralSense) / (dist x 0.6 + 0.4) x DetectUsScale >= 1`), all `[rules.ai] stealth_*` tunables; stock-dedi emission reality pinned in entity-ai.md: the C2S sound relay is audio-only on a dedicated server (PlaySoundAtPositionServer skips NotifyNoise when IsDedicatedServer; NetPackageEntityStealth server branch ignores client noise levels), so the model's inputs are server-side sounds only (explosions/mines/animals/minibike via Audio.Manager.SignalAI / GameManager.explode) - none player-attributed in the sim yet, the ring stays ready as those sources land. Missing (recorded): the per-position world-light legs of the CanSeeStealth sight gate (block light, moving lights, moon/shade - slice 1 day/night ambient ships 2026-08-26 via world/sky.zig driving the stealth S2C light byte and the sleeper crouch leg; the CanSeeStealth sight-light threshold leg awaits per-class SightLightThreshold + block light), the GetSleeperDisturbedLevel wake/groan ladder, smoke/smell stealth) |
