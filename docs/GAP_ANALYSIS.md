...
| **Total** | **298** | **2** | **0** | **300** | Two PARTIAL rows with named shortfalls: the perk/attribute passive-effects VM (§10) and the join-burst tick budget (§11, 2026-08-29). Round 18 (2026-09-11) closed the armor-set activation chain inside §10 (`ArmorGroupCount` + the full triggered-row gate + buffStatusCheck02 driven from data); round 21 (2026-09-12) closed the buffs.xml gate vocabulary (`EntityTagCompare`, the `Stat::Max`/`Stat::ModifiedMax` family and `IsNight`, so all 43 gated tracked buff rows resolve); rounds 23-24 added the equipped/held item fold and `IsEquipped`, and `EntityHasMovementTag` from the reported movement state; 2026-09-15 added `RandomRoll` (Random seed), `PerksUnlocked` (child-perk sum), `IsStatAtMax` (0.1 threshold), `InSafeZone` (false, no Twitch), `IsAttachedToEntity` (vehicle seats) and `EntityTagCompare target=other` at damage time (Spectral Grace deflects + recharges; Preacher armor gains its zombie-filtered PDR) and `onSelfBuffFinish` at expiry (injury cooldown chains, Grace recharge clear; the remaining finish actions are Twitch/particles/debug with no server consumer) and `AddOrRemoveBuff` as a gate toggle (20 weather/hazard rows) and `onSelfBuffStack` on re-add (93 harvest/ready-flag rows) and `onPerkLevelChanged` on purchase (4 point-chance rows) and `onOtherAttackedSelf` on the victim in both damage paths (19 concussion/fatigue/PackMule rows) and `onSelfKilledOther` on the killer's buff + perk rows at the C2S kill award (DeadEye/Berserker adds, the stamina/health ModifyStats refunds; delayed rows apply immediately, no stock row on this trigger carries `delay=`) and HealthLoss/HealthGain (106/107) scaling on the applied C2S damage leg (`Stat.Tick`: fatigued +.1 takes more, toughness -.5 takes less) and BuffResistance (197) immunity on buff adds (`EntityBuffs::HasImmunity`: PainTolerance resists stuns, deterministic entity+buff draw) and HealthSteal (167) healing on landed C2S hits (`ProcessDamageResponse`: damage x GetValue, NightStalker book behind night/crouch/sleeping-victim gates) and ExperienceGain (193) scaling on kill XP (`AddKillXP`: class XP x the killer's held-item GetValue; no stock held rows ship it, Twitch-only) and PlayerExpGain Kill rows on kill XP (`AddLevelExp` useBonus: TwilightThief +.05 at night; the `IsLocalPlayer` SavageReaper row refuses on a dedi) and `target="other"` victim-directed AddBuff/RemoveBuff rows on the attacker hit event (shotgun stuns, cripples, bleeds land on the victim zombie; parsed into parallel other lists, applied through the victim sink) and attacked-event ModifyStats refunds (MachineGunner stamina, AdrenalineHealing health: evaluated but never applied until now) and attacked-event self ModifyCVar rows (PummelPete/Gunslinger combo counters rise per hit through the shared evaluator; victim-directed bleedCounter rows stay recorded-only, zombies carry no cvar store) and victim-event symmetry (other-sink + other-apply on the victim hit event; today its rows are all self-directed) and kill-event other-apply symmetry plus the FortitudeMastery kill RemoveBuff path (bleed clears on kill through the flagged-removal reap) and the victim PackMule progression fold (the victim event never evaluated the victim s own perk rows) and the RandomRoll seed finalizer (sequential entity-id x tick seeds clustered near 97% out of xorshift, so the 5% PackMule gate never passed; SplitMix64 finalizer now spreads them) and onSelfLeaveGame on disconnect (storm/harvest/smell cleanup removes fire in the session-drop path before destroy) and onSelfDied on player death (infectionCounter set, aim/draw-buff removes fire in the hp-replicate death path before the game_on_death sequence) and TargetRange target=other on the hit event (self-to-victim distance; PistolPete/LimbShot close-range gates resolve instead of refusing) and StatComparePercCurrentToMax target=other on the hit event (victim Health fraction/max; PulverizingFinishers <=30% and PartyStarter full-HP gates resolve) and EntityTagCompare target=other on the kill event (victim class tags; FortitudeMastery Dentist silver/gold proc on walker/crawler/bandit kills, proven end to end) and target=otherAOE start rows fanned out around the holder (stock centers the range cube on Self; SledgeSaga cripple morale lands on zombies within 3 m, proven end to end; admin RingOfFire selfAOE stays recorded-only) and target=selfOtherPlayers party share on progression update (CharismaticNature level + group buff fan out to party/ally members with each member s own cvar store supplied, proven end to end) and CVarCompare target=other via the member store supplier and target=other ModifyCVar/RemoveCVar writes on the hit event (victim player store supplied; PistolPete bleedCounter lands, proven end to end) and per-entity cvar columns (lazy like buffs: zombies own a bleed/sprain store exactly like stock EntityBuffs; PistolPete 9 mm bleedCounter escalation + bleed buff land on zombie victims, proven end to end) and CallGameEvent buff rows through the sequence runner (Dentist silver/gold action_give_* supported via client-performed AddItems; grant response asserted end to end) and victim-directed ModifyStats routing (StatMod carries target_other; euthanizer set-to-kill skips self folds and lands in the victim applier for Health/Stamina set/subtract/add) and comma buff=/cvar= list fan-out on recorded-list appliers (stock splits both into name arrays; multi-name RemoveBuff rows like the splint ladder now remove every listed buff, proven end to end; RemoveCVar comma lists split too) and immediate Food/Water ModifyStats deltas on buff start/update (puking drains 50 water once; previously only Health adds applied) and immediate Stamina ModifyStats deltas on buff start/update (radiation pool drains 20 per due update) and immediate Health subtract rows once per firing event (infection04 kill blow; ModifyStats value=@cvar resolves at execute time per the IL) and victim-directed ModifyStats routing (StatMod carries target_other; euthanizer set-to-kill skips self folds and lands in the victim applier for Health/Stamina set/subtract/add) and cvar-valued Health adds healing per update (buffHealHealth @medRegHealthIncSpeed drains the medical pool; proven end to end) and shared-applier stat deltas on finish events (spawn-heal +30000 restores; hit/kill sites keep single-apply via applyKillMods with an apply_stats=false gate) and delay= ModifyStats deferral on the kill event (21 stamina refunds at 1.0 s queue onto bounded per-client slots, drained on the survival tick; the stale no-delay survey note is superseded) and kill-event foreign-input symmetry (victim ledger/alive/buffs/corpse/sleeper/HP/cvars supplied like the hit event; no stock kill row gates on them yet) and kill Berserker grant (StrengthMastery 5 + club-family held item; proven end to end) and died-path radiation self-remove (extends the died test; bleeding/infection/aim rows ride the same path) and damage_type element parse + RemoveAllNegativeBuffs cure (near-death regen flags every typed buff; proven end to end with typed/untyped witnesses) and the held-item triggered surface (items.xml triggered_effect rows parsed per item with their effect-group gates and Extends inheritance; the held weapon s onSelfAttackedOther fire at the hit event - torch burn proc lands on the victim, proven end to end) and the zombie-attacker hand rows on the victim event (the attacker class HandItem s onSelfAttackedOther run against the player victim with foreign fills remapped; infection escalates only when the victim is already infected per the group gate, proven end to end) IsSecondaryAttack supplied (IL=65 resolves to the held item declaring Action1, a weapon shape not the swing type; batter-up/sledge club power-kill stamina refund gates now pass, proven end to end) and fireOneBuff weighted single-pick (zombie-fist wound rows pick ONE of the seven weighted buffs per hit like stock Entity.rand, instead of all; deterministic per-target net id; proven end to end)) 2026-09-19 consumable-item gap closed: the dedicated server fires the eaten item's onSelfPrimaryActionEnd (ItemActionEat), so the consumable grants were missing - beer/caffeine stamina buffs, drug water/DR buffs, goldenrod dysentery cure, and the fire-extinguish contract all never landed. The eat path now fires the item's rows with the food/water/HP pool cvars suppressed (EatProps owns the restore; writing the pools would double-drain through buffProcessConsumables); the fire-extinguish gating (HasBuff buffIsOnFire) is proven both ways. 2026-09-19 ranged ray-hit gap closed: ItemActionRanged fires onSelfPrimaryActionRayHit (MinEvent 25) on a hit, so the ranged bleed skills (perkAgilityMastery 3 / perkPerceptionMastery sniper bleedCounter + bleed) were never firing. The C2S damage claim cannot tell melee from ranged, so the held weapon's `ranged` tag now picks the ray-hit path (shared hit-row passes) over onSelfAttackedOther; the agility pistol bleed lands on the zombie and the melee path is proven not to. 2026-09-19 level-curved ModifyCVar + consumable duration closed: the eat-path writes now merge to the player store except the food/water/HP pool names, so the consumable buff's own duration cvars seed ($buffBeerDuration) and can expire; and ModifyCVar value curves index valueList[level-1] from the changed perk's level (stock Execute IL), so the Physician/SlowMetabolism/Parkour curve rows write their real per-level values (Physician 2 = heal speed 2.5). Together with the eat-path item rows this completes the Physician heal-speed chain (bandage sets medRegHealthIncSpeed from $medicRegHealthIncreaseSpeed). 2026-09-19 respec closed: Grandpa's Forgetting Elixir's ResetProgression row (reset_skills=true) was never fired - no respec handler existed. The eat-path item rows now parse ResetProgression, record it, and resetProgression refunds CalculatedCostForLevel for every perk/attribute level above base (perks->0, attributes->1, books/crafting untouched, stock Progression::ResetProgression IL=113), banking the refund in SkillPoints. Proven end to end (attStrength level 2 + PummelPete 3 reset with the full refund). 2026-09-20 residual re-sweep: no secondary-only use rows (every eatable's buffs ride primary), explosion min-events are Twitch-only, refused buff-actions are all client FX/challenges (Ragdoll, AwardChallenge, GetBuffDuration, SetHeldItemJammed), and the ammo onProjectileImpact burning (molotov/arrow) is wire-blocked - the damage claim carries no ammo id and the projectile is client-simulated - a genuine residual requiring RE + wire work, not a single-round fix. 2026-09-20 consumable expiry proven end to end: SlowMetabolism sets $MetabolismDuration, the buff's update rows drain the seeded $buffBeerDuration by MetabolismDuration/2, and the RemoveBuff gate (LTE 0) removes the buff server-side - the full drug/beer grant -> duration -> wear-off lifecycle now matches stock. 2026-09-20 damage-apply events closed: onSelfDamagedOther (attacker) and onOtherDamagedSelf (victim) were never fired - the Agility leg-cripple, Physician stun-baton euthanizer and BarBrawling rage all missed. The landed-hit paths now fire the damage-apply pass alongside the attack pass (both melee and ranged), and the victim path fires other_damaged_self; rage proven end to end (knuckles + BarBrawling 6 rage grants on taking a hit). 2026-09-20 combat-entry closed: onCombatEntered never fired, so the Enforcer Criminal Pursuit, Batter Up Stealing Bases and Urban Combat Adrenaline stamina buffs (two with tracked passives) missed. A per-client combat idle window (5 s at 20 TPS) now refreshes on every dealt/received damage and fires the combat_entered progression rows on the out-of-combat edge with the held weapon's tags supplied; magnum + Enforcer 1 proven end to end. 2026-09-20 fall-impact closed: a self-fall damage claim (failing dtype, victim == the player) is the landing impact, and onSelfFallImpact never fired - the leg-injury escalation ($legHurtCounter, buffLegGetsWorse) and buffPlayerFallingDamage were missing. The damage handler now fires fireFallImpact with the claimed amount as the _fallSpeed proxy (stock records the impact velocity); sprained-leg falls escalate the counter and grant the fall-damage buff, proven end to end. 2026-09-20 onSelfBuffRemove ordering fixed: the survival pass evaluated .remove rows BEFORE the update pass, and buff.tick reaped the flagged slot before the next survival pass, so onSelfBuffRemove rows for buffs removed by a RemoveBuff action (cvar cleanup, remove-chains) never fired. A post-update remove pass now fires them (de-duped against the pre-branch); the consumable's duration-cvar cleanup is proven end to end. 2026-09-20 infection escalation proven: buffInfectionMain's update row adds @_InfectionRate to infectionCounter every update tick server-side, so the infection worsens over time and gates the stage/cure rows (the write side was already proven by the zombie-fist infectionCounter test). 2026-09-20 TriggerHasTags closed (part of the church-bell feature): the requirement was unsupported, refusing the bell spawn gate. New kind + ctx trigger_tags + eval read the event's tag set (the damaged block's tags, per the buffs.xml doc comment the trigger is specific to onSelfDamagedBlock/onSelfDestroyedBlock); unit-tested. Also settled the ammo burn: the corpus (items.md Catapult/Launcher) shows projectiles are client GameObjects with ProjectileMoveScript, not server entities - the onProjectileImpact burn is client-resolved, not a server gap. 2026-09-20 church-bell mechanism in place: blocks.xml Tags now parse onto the block def (519/6653 rows; churchBell carries the churchbell tag, proven), and onSelfDamagedBlock fires in the block-damage path with the damaged block's tags supplied to TriggerHasTags. The remaining content pieces are the SpawnEntity game-event action and the sequence's RandomRoll gate (a gated sequence parses unsupported today), a coherent next feature. 2026-09-20 church-bell horde complete: the SpawnEntity game-event action (SleeperGSList x4, 10-20 m arc, aggressive) + the sequence RandomRoll gate (LTE 99) now parse and run, so a bell block hit with the Duke note fires block_bell_spawn and spawns the horde - proven end to end (zombie count rises). This was the last identified server-side content gap. 2026-09-20 W2b premise measured moot: the apm shows chunk_gen at 4.7us mean (the round-8 te_scan skip + pointer-stable store already took the cost out), so async chunk gen would move microseconds off the tick. The tick overruns under 8-way concurrent load come from the replicate fanout and the paced spawn-area drain (both already serialize-once, dirty-gated and budget-split); the join behaviour itself is 100% (join_fail=0). W2b is a perf optimization with no behaviour delta, not a fixable gap. 2026-09-20 final assessment: seventeen rounds of exhaustive audits across every surface (buff/item/progression triggers, actions, requirement vocabulary, foreign kinds, C2S packages, DIVERGENCES, weather/storm, block events, perf) surfaced and closed every detectable server-observable gap with a verified test (consumables, ray-hit bleed, level curves, respec, damage-events, combat-entry, fall-impact, onSelfBuffRemove ordering, church-bell horde, TriggerHasTags/block tags). The remaining items are measured non-gaps: W2b chunk gen is 4.7us per chunk post the te_scan skip (perf only), the no-ship legs are client-side by design, and the only failing test is the in-flight webui writer renderLoginLockout (not this scope). The suite over the server scope is green. This supersedes the 2026-09-19 completion assessment, which was premature. and cvar-scaled HealthChangeOT passives proven (bleeding drains bleedCounter/2 HP per second through the active-buff fold; the internal-bleeding row shares the path) and the Harvesting-tagged PlayerExpGain fold proven (harvest passes "Harvesting" through awardXpTagged; MotherLode 1 -10% scales a 100 XP harvest to 90). §11 join-burst loadgen evidence refreshed 2026-09-19 on the ReleaseFast binary (4-way and 6-way join-burst cohorts at the stock 8-player cap: pass 4/4 and 6/6 fail=0 passRate 100%; 8-way and 12-way cohorts show every bot reaching PASS joined with an assigned entity and rejoining freely - the lower cohort pass% is the loadgen life/rejoin end-state tally confounded by world deaths, and the 12-way transport ConnectionFailed entries are the 127.x multihomed-bind concurrency artifact re0 sent0 pre-join, neither a join-burst failure; the join behaviour itself is 100%) and a clean full-cap cohort (2026-09-19, stock 8-player cap, no deaths, no transport failures: JOIN_SUMMARY total=8 pass=8 fail=0 passRate 100.00% -- the cleanest run at the stock max). `ItemHasTags` (98 rows, the top unparsed kind) gates only untracked/weapon-local passives (StaminaLoss, LootProb, DismemberChance, HeadshotDamageModifier), so parsing it changes zero tracked outcomes. Requirement vocabulary closed 2026-09-16 (`59 of 59` stock kinds evaluate (TargetRange + WasAlive closed 2026-09-19), including dedi-false/stub fills for Unity/client-only gates and `IsIndoors` false until AmountEnclosed is tracked). Foreign-target exhaustion re-audited 2026-09-19: the only remaining target=other kinds outside the closed set are buffs.xml StatCompare on buffShowQuestLimitReached, which sits inside the stock XML comment block (never parsed), and the biome hazard/storm buffs whose rows are client-UI (XUi reads BiomeDefinition.Buff) or prob=0 weather rows that never trigger from the weather sim; storm/Twitch weather never grants a buff server-side, so no server behavior is skipped. 2026-09-19 completion assessment: the behaviour scope is exhausted - every server-observable path (triggered buffs and gates and foreign suppliers, item and zombie hit surfaces, passive folds, kill hit victim died events, party share, cure-all, fireOneBuff, delay, XP tags, cvar stores) is covered by a green test; the residual PARTIAL tags cover only client-side computation and UI (untracked passives scaling client damage numbers, sounds, particles, screens, unfired admin Twitch weather triggers, storm rows prob=0), the XML-comment example row, and the W2b async-chunk-gen perf item (explicitly server internals, out of the behaviour scope). make check is red only on an in-flight webui writer lint (login.html aria-invalid template), not behaviour code. What keeps it PARTIAL is the remaining foreign-target kinds without a supplier (Physician `ProgressionLevel target=instigator` closed 2026-09-16 via BuffInstance.instigator_id → instigator_levels; `addCatalogBuff`/`applyTriggeredBuffs` take instigator_id; `ProgressionLevel target=other` closed 2026-09-16 via other_levels on damage folds (BatterUpMetalChain); `IsAlive target=other` closed 2026-09-16 via other_alive on damage folds; `HasBuff target=other` closed 2026-09-16 via other_live_buff on damage folds; `IsCorpse`/`IsSleeping` target=other closed 2026-09-16 via other_is_corpse/other_is_sleeping on damage folds; TargetRange target=other closed 2026-09-19 via other_distance on the hit event; WasAlive target=other closed 2026-09-19 via other_alive (victim alive on a hit); other foreign kinds still refuse) and the non-tag foreign kinds (all gate untracked passives, unfired triggers, or attacker-side `target="other"` damage rows like the spear-vs-downed bonus that scale the client's finished damage number the server accepts), the untracked passive names, and the surviving row-action legs (a 2026-09-15 survey: the buff-lifecycle triggers consume AddBuff/RemoveBuff/AddOrRemoveBuff/ModifyCVar/RemoveCVar/ModifyStats; every other action rides item/combat/Twitch/weather triggers with no server-side consumer - magazine eat SetProgressionLevel/GiveExp closed via grantMagazineRead; AddHealth on buff lifecycle closed 2026-09-16; storm/Twitch triggers still unfired; particles/sounds/screens). Death/kill counters promoted to WORKS 2026-09-08 (client-accrued accumulators live in DIVERGENCES §2). Chunk-pointer stability closed 2026-08-30 by the pointer-stable chunk store |
...
21. **Progression: build a buff runtime + the passive-effects VM.** `WORKS`
    ...
    **FULL ROW GATING + ARMOR-SET CHAIN 2026-09-11 (round 18)**: every
    ...
    restores 100), with the changed maxes folded into the survival sync. Buffs persist across restart (ZPV12 progression tail) and re-apply through the VM
...
22. ~~**Progression: simulate survival.**~~ **PARTIAL → DEPLETION LOOP SHIPPED
    ...
    **FoodChangeOT/WaterChangeOT consumer closed 2026-09-17**: with a game-dir
    ...
    moonshine rows surveyed and ready). The 119/127/120/126 rows scale the
    ...
    **HealthOT water gate closed 2026-09-18** (`UpdatePlayerHealthOT`
    ...
    closed the same day (IL=139: positive leg below full stamina, scaled by
    max(water%, 0.2)).
...
- **Air-drop crate NavObject markers** `WORKS` `(2026-09-10)`
  ...
  ../../7dtd-engine-research/docs/gameplay/map-objects.md:306); every other marker family is client-derived from synced
  ...
  sequence, RE ../../7dtd-engine-research/docs/network/protocol.md:317). `sendAirDropNavObjects` does the same, and the
  ...
  `EnumMapObjectType.SupplyDrop` 13, channel 192; RE ../../7dtd-engine-research/docs/entities/aidirector.md:84), and
...
- **Force-unlock: re-lock, failed transaction, disconnect** `WORKS` `(2026-09-10)`
  ...
  2. **A failed inventory transaction.** `TransactionRequestServer` (IL=46, RE
     ../../7dtd-engine-research/docs/network/protocol-packages.md:1263) logs and force-unlocks on a failed apply. The
...
- **Force-unlock on disconnect** `WORKS` `(2026-09-10)`
  ...
  ../../7dtd-engine-research/docs/meta/dedicated-leftovers.md:173). The `locking = false` branch routes the client
...
- **traders.xml trader_item_group parsing with nested group refs** `WORKS`
  ...
  child refs recursively with a depth limit and a visited set. The earlier note
  here said group refs are skipped; they are expanded, with a test against the
  real stock file.
  *Anchors:* `src/assets/traders.zig:114-177`, `:54-82`, `:183-201`,
  `$game/Data/Config/traders.xml` lines 1179-1194
...
- **traders.xml root economy attributes** `WORKS` (2026-08-07, RE'd 2026-08-08):
  ...
  *Anchors:* `src/assets/traders.zig` root row, `src/server/game/step.zig`
  ...
  `coinItemId`, `$game/Data/Config/traders.xml` line 3, `asm.il:1397236-1397257`,
...
- **Haggling / barter perks** `WORKS` (was `PARTIAL (waived)`; closed
  ...
  this one.
  *Anchors:* `$game/Data/Config/progression.xml` lines 3064-3065 and 3084,
...
- **Vending machines** `WORKS`
  ...
  **Owned-machine map markers ship 2026-09-10**: the join
  ...
  `../../7dtd-engine-research/docs/world/save-region.md:982`), which was a hardcoded `0` count, so the client
  ...
  *Anchors:* `src/world/vending.zig`, `src/wire/stock_te.zig:789-881`
  ...
  `$game/Data/Config/blocks.xml` line 51104, `$game/Data/Config/traders.xml` line 1472
...
- **Blood-moon weather override** `WORKS`
  ...
  *Anchors:* `src/world/weather.zig:106`, `:125`, `src/server/game.zig`,
  `src/assets/biome_layers.zig:757`, `$game/Data/Config/biomes.xml` line 190
...
- **Horde spawn composition** `WORKS` (2026-08-20 reconciliation)
  ...
  *Anchors:* `src/ecs/aidirector.zig:400`, `:521`, `:644`, `src/server/game.zig`
  `pickStageGroup`, `$game/Data/Config/gamestages.xml` line 4428,
  `$game/Data/Config/entitygroups.xml` line 15809
...
- **Wandering horde / screamer heat** `WORKS`
  ...
  *Anchors:* `src/ecs/aidirector.zig:159`, `:168`, `asm.il:409351`,
  `$game/Data/Config/gamestages.xml` lines 1582 and 3458
...
- **Loot content per container** `WORKS`
  ...
  a timing difference that is now fixed (2026-09-12): the chunk/prefab scan
  sizes the grid only and the roll happens on the open path with the **opener's**
  loot stage (`LootManager.LootContainerOpened`, `bTouched` + `worldTimeTouched`
  stamped at open), so the opener's state is available to the gates.
...
- **AIDirector world clock, day/night, blood-moon night detection** `WORKS`
  ...
  minute day, RE ../../7dtd-engine-research/docs/admin/server-lifecycle.md:168), the same value the GameStats blob and
...
- **Sleeper group name to entity class resolution** `WORKS` `(2026-08-22 re-audit)`
  ...
  *Anchors:* `src/server/game/sleeper.zig:109-110`,
  ...
  `$game/Data/Config/gamestages.xml` line 153, `Data/Prefabs/POIs/*.xml`
...
- **Target sensing** `WORKS` `(2026-08-22 re-audit)`
  ...
  *Anchors:* `src/ecs/systems.zig:104` canSensePlayer, `:137` viewHalfDeg,
  ...
  `$game/Data/Config/entityclasses.xml` lines 678-679, `asm.il:430171`
...
- **Wandering hordes** `WORKS` (pack path-walk residual)
  ...
  *Anchors:* `src/ecs/aidirector.zig` (`wandering_next`, `nextWanderingTime`,
  ...
  `asm.il:416218`, `$game/Data/Config/gamestages.xml` lines 1582 and 3458
...
- **Screamers and the activity heat map** `WORKS` (heat feed residual)
  ...
  *Anchors:* `src/ecs/aidirector.zig` (`heat`, `notifyActivity`, `tickHeat`,
  ...
  `asm.il:416218`, `$game/Data/Config/blocks.xml` line 28086 (forge 6)
...
- **Corpse dwell time (TimeStayAfterDeath)** `WORKS` (corpse harvest residual)
  ...
  *Anchors:* `src/ecs/world.zig` (`damageFrom` corpse branch, `sweepCorpses`),
  ...
  `$game/Data/Config/entityclasses.xml` lines 692-693
...
- **Zombie / animal death loot bag contents** `WORKS`
  ...
  *Anchors:* `src/assets/entities.zig` load, `src/ecs/world.zig` damage gate,
  ...
  `$game/Data/Config/entityclasses.xml` line 689, `$game/Data/Config/loot.xml` line 9928
  *Anchors:* `src/assets/entities.zig:245-247`, `src/server/game.zig`,
  `src/assets/loot.zig:119-123`, `$game/Data/Config/entityclasses.xml`,
  `$game/Data/Config/loot.xml` line 9927
...
- **Player death loot bag (DropOnDeath)** `WORKS` `(2026-08-22)`
  ...
  *collected*, so a player who died again before collecting dropped nothing at
  ...
  that could destroy one. The owner's markers ride the
  player record since the same day (ZPV13, magic byte 'D': `n:u8 | n x (x,y,z
  i32)` after the skill tail), so a restored bag still shows on its owner's
  map instead of lying there unmarked.
...
- **Client to server progression blob (NetPackagePlayerStats)** `PARTIAL (waived)`
  ...
  client-echoed. Re-checked 2026-09-02: the split is real and deliberate, so
  the waiver stands on the C2S ingest leg only.
...
- **Perk / attribute passive effects applied to gameplay** `PARTIAL`
  ...
  The bounded passive-effects VM shipped 2026-08-27: level-scaled perk passives
  fold over the tracked stats (armour resist + `HealthChangeOT` regen),
...
- **NetPackageAddRemoveBuff relay and emission** `WORKS` (2026-08-06)
  ...
  ../../7dtd-engine-research/docs/gameplay/buffs.md:195), but zdtd's respawn cleared the set directly and discarded the
...
- **Food / water application on eat** `WORKS` `(2026-08-22)`
  ...
  *Anchors:* `src/ecs/inventory.zig:285-306` applyEatProps,
  `src/assets/items.zig:444-456`, `$game/Data/Config/items.xml` lines 20015-20029,
  `$game/Data/Config/buffs.xml` line 8477
...
- **Malleable world geometry (ADR 0036, 2026-08-28)** `WORKS`
  (1) `[rules.geometry]` elevation projection: the terrain sources (flat /
  baked DTM / proc) feed `clamp(height_offset + height_scale * elev_m, 0,
  ceiling)` with `sea_level` shaping the flat fill and the DTM fallback;
  identity at stock defaults (byte-identical vanilla worlds). (2) The `[wire]
  profile` column-height dialect surface was removed (2026-09): zdtd emits
  only the stock 256-tall format, so there is no `[wire] profile` config and
  no tall-512 synthetic dialect. The `WireProfile` struct survives as the
  save/chunk carry of the fixed stock height (ADR 0036 cites it).
  *Follow-ons:* RealEarth's actual dialect is RE-gated (their patched wire
...
- **Deco density (biomes.xml probabilities)** `WORKS` (superseded 2026-08-08,
  ...
  *Anchors:* `src/world/subbiome_noise.zig`, `src/server/game/deco.zig:19`,
  `src/server/game/join.zig:55`, `$game/Data/Config/biomes.xml` lines 489-507
...
- **type="prefab" decorations** `WORKS` `(2026-08-07)`
  ...
  *Anchors:* `src/wire/stock_deco.zig:154-159`, `$game/Data/Config/biomes.xml` lines 283,
  `310` and `494-495`
...
- **Land claim keystone registration, edit deny, durability modifier** `PARTIAL`
  ...
  **Residual (found 2026-09-09): the durability modifier applies to player
  ...
  chew and dig paths, `docs/GAME_OPTIONS.md:99-101`). So the claim modifier is a
...
- **Land claim replication to the client (lpBlocks)** `WORKS`
  (re-evaluated 2026-09-02: the blocking RE gap is closed)
  ...
  The waiver was blocked on not knowing the `List<Vector3i>` count-vs-list
  layout. That RE now exists: `PersistentPlayerData.Write` (IL=205) is
  documented in RE `server-lifecycle.md` 6.1 as `lpBlockCount : i32` followed
...
- **Auth-state timeout (half-open connection reaping)** `WORKS` `(2026-09-07)`
  ...
  *Anchors:* `src/server/game/tick.zig` (`reapStalePeers`),
  ...
  auth age`), RE `../../7dtd-engine-research/docs/network/network.md:810`
...
- **Steam / EOS master-server registration** `PARTIAL (waived: direct-IP parity)`
  ...
  *Anchors:* `src/server/serverinfo_tcp.zig`, `$game/serverconfig.xml` line 16
...
| `RequestToSpawnPlayer` | PARTIAL (2026-08-26: chunkViewDim i16 parsed (stock read IL=13); the client's PlayerProfile blob is skipped - the server owns the profile, the client's copy is display-only. **2026-09-04:** the nearEntityId tail read was removed. It took the last four bytes without parsing the variable-length profile, so on a short body it reinterpreted chunkViewDim itself, and no server code consumed the value. Reading an unused field from an underived offset is worse than not reading it) |
...
| `NetPackageEntityWaypointList` / map markers | PARTIAL (was scored SHIPPED on the 2026-08-26 re-audit for the wrong reason - the client-local receive half; corrected to PARTIAL 2026-09-06 when the server-sender sweep flagged the missing send, then closed the same day. The `ProcessPackage` IL=22 reading is right - it is client-local, `GetPrimaryPlayer` plus the local `WaypointCollection` - but that only describes the receiving half, and the row treated "no C2S role" as "nothing to do". The server *sends* it: `VehicleManager.UpdateVehicleWaypointsForPlayer` (IL=69) walks the vehicle ECD list for `belongsPlayerId` and, when that player is remote, ships the id/pos pairs so their map shows where they parked; `DroneManager.UpdateWaypointsForPlayer` (IL=96) is the same for drones. Builder shipped 2026-09-06, re-scored PARTIAL 2026-09-10: the stock body (`listType` i16 Vehicle=0, count, `(entityId, Vector3)` pairs) ships from the join bundle via `sendVehicleWaypoints` and filters on `Vehicle.owner_slot`, but no production path ever sets that field. Vehicles reach the world through worldgen (`game/init_world.zig`) and the admin console only; there is no vehicle placement package, so `entityThatPlaced` has no zdtd counterpart yet and every live vehicle is unowned. The list is therefore always empty in play and the sender is exercised only by `game/tests.zig:4126`, which assigns the owner by hand. Closing this needs the placement path, not another builder. Drones have no sim entity, so the drone list type stays unsent - stated with the subsystem, not the package; the day drones exist the same builder carries them) |
...
| `NetPackageEntityAwardKillServer` | SHIPPED (both directions as of 2026-09-06; the S2C half was missing and the row said SHIPPED anyway until the server-sender sweep flagged it. The C2S half of that row still stands - the kill credit is server-computed from the DamageEntity death: questOnZombieKilled, questKillForParty, killXpAward, the AddScoreClient counter - and accepting the client's report would double-credit, so the accept-and-drop (DIVERGENCES 1.12) is right. What the row missed is that stock also **sends** this package: `GameManager.AwardKill` (IL=27) ships `Setup(killerId, killedId)` on channel 192 whenever the killer is a remote entity, and the receiving client runs `QuestEventManager.EntityKilled` (IL=24) to fire its local `EntityKill` event. That event is what `Challenges/ChallengeObjectiveKill` and `ChallengeObjectiveKillByTag` subscribe to, so without the S2C send a player's kill challenges never advanced on a zdtd server. Closed by `awardKillNotify` (`server/game/player.zig`), called from both kill sites - the melee/ranged DamageEntity death and the ExplosionInitiate death - and sent only to the killer's own connection, as stock does. Contradicts the "no server-required challenge wire exists" claim on the Challenges row: this is that wire) |

#### Combat / hazards
| Package | Priority |
|---|---|
| `NetPackageDamageEntity` full field semantics | PARTIAL (the field-level scoring below is unchanged and still WORKS. The send half landed 2026-09-06: both server damage paths (melee/ranged C2S and the explosion victim loop) now fan the applied hit to the victim's trackers via `broadcastNear` with the server-side attacker id and the capped strength, matching stock's `SendPacketToTrackedPlayersAndTrackedEntity` in `ProcessDamageResponse` (IL=86). The explosion arm carries source External and the stock `ExplosionData.DamageType` default Heat, which the initiate body does not transport. Downgraded from the old "the builder emits" wording because that described a builder no send path called; the dismember row's "emits no S2C NetPackageDamageEntity at all" is stale too. What the clients do with it - the hit reaction and the dismember/cripple reads - is their side. Stock does send it: 2026-08-26 re-audit, still accurate for the body itself: the builder produces the FULL stock body - all 31 fields in read-IL order (entityId, damageSrc/Typ, strength, hitDirection, hitBodyPart, movementState, pain/fatal/critical, attacker, dirV, blockPos, hitTransform, uvHit, damageMultiplier, random, ignoreConsecutive/buff/dismember/cripple/crawler flags, bonusDamageType, StunType/Duration, bFromBuff, ArmorSlot/Group/Damage, optional ItemValue); parseDamageHead consumes the head and validates the acted-on fields (entityId/source/type/strength/fatal/trapKillXP). Correction 2026-09-06: calling the remaining flags "hit-info cosmetics" understated one of them - see the dismember row below) |
| `NetPackageSetAttackTarget` | SHIPPED (2026-09-06: found by re-deriving the server senders from the v3.2.0 IL, the mirror of the C2S sweep. Stock fans this out of every server-side target change - `EntityAlive.SetAttackTarget` IL=70 sends `Setup(entityId, target ? target.entityId : -1)` and the `OnUpdateLive` expiry IL=363 sends -1 - and the client stores it as `attackTargetClient`, which is what `GetAttackTargetLocal` returns for a remote entity (drone beam targeting, DynamicMusic threat level). zdtd picked targets in `ai.target_id` and published nothing, so every remote zombie read as untargeted. Now `tickAttackTarget` (`server/game/tick.zig`) diffs the per-tick value against the last published one per slot, generation-pinned like the look-at cache, and broadcasts the stock 8-byte body to peers in interest range; a target that is no longer alive publishes stock's -1) |
| Dismember / cripple / crawler | SHIPPED (was MISSING; closed 2026-09-06 by the movement audit below. no server-side sim, undocumented until 2026-09-06). Stock decides these on the authority, not from the packet: `EntityAlive.ProcessDamageResponse` rolls `CheckDismember` (IL=125) against a source-dependent chance, sets `Dismember`, and a leg hit additionally sets `TurnIntoCrawler` or, below the crawler threshold, `CrippleLegs` (RE `combat-damage.md` §"CheckDismember"). Ignoring the inbound `dmg_dismember` / `dmg_cripple_legs` / `dmg_turn_into_crawler` bits is therefore right - a client must not declare a limb off - but zdtd never rolls them either, and it emits no S2C `NetPackageDamageEntity` at all, so nothing could carry the result. Visible effect: zombies never lose limbs and a leg hit never produces a crawler. Data half landed 2026-09-06: `EntityDef` now carries `dismember_head/arms/legs`, `leg_cripple_scale` and `leg_crawler_threshold`, parsed from entityclasses.xml with Extends resolution and verified against the stock file (template 1/1/1 + 2 + 0, feral .7, radiated .4; the EntityClass cctor defaults are the same). The remaining inputs are mapped, not guessed. `GetDismemberChance` (IL=128): region multiplier from the new fields, then `EffectManager.GetValue(143 = DismemberSelfChance)` for the attacker's perk/buff bonus, then `DamageSource.DismemberChance` (the weapon's passive-144 value, clamped to 100; at >= 100 the roll is skipped and the chance is flat 100), times `damagePer` which ProcessDamageResponse always passes as 1.0, compared directly against the 0..1 draw (no /100). Killing blows (fatal or strength >= hp) pre-scale first: head at fraction >= 0.2 gets max(c*0.5, 0.3), else max(c*0.5, 0.5). The roll itself is `entity.rand`, and the leg branch additionally reads `LegCrawlerThreshold` / `LegCrippleScale` plus body flags 4096/8192 for left/right. The roll landed 2026-09-06 (`World.rollDismember`, melee C2S path): leg gating on alive + awake, region multipliers off the new class fields, `GetDamageFraction` as damage/maxHP, a deterministic per-hit draw, the crawler threshold arm and the scaled cripple arm with the 0.05 floor and the left/right masks, and the outcome bits ORed onto the fanned-out S2C body at the stock flag offsets (Setup IL=235 verified: cripple 0x2, dismember 0x8, crawler 0x200). Corrected 2026-09-06: the first version of the roll multiplied the whole chance term by a 0 attacker bonus, zeroing every roll. Stock's twitch buffs pin 143 to 0 precisely because the unbuffed value is the region multiplier, not zero, so the term is weapon x damagePer x (mult + perk143bonus) with the bonus reading 0 until the perk fold lands - and the roll fires in vanilla. One remaining dormancy: the `DismemberSelfChance` perk fold itself, and the weapon side reads the new `ItemDef.dismember_chance` (first untagged `DismemberChance` row; perk-tagged rows belong to the 143 side, not the weapon). Movement audit 2026-09-06, and it dissolves the remainder rather than adding work. There is no sim-speed slow to implement: `GetMoveSpeed` / `GetMoveSpeedAggro` take no walkType or cripple input (both are pure `EffectManager` passive reads), and no EAI task or zombie method branches on `ShouldBeCrawler` / `IsCrippled`. The effects are: crippled + leg hit + alive with walkType strictly between 5 and 20 sets walkType 5 (the limp), `ShouldBeCrawler` runs `SetupCrawler` (walkType 21, MaxHeight 0.5, `HandItemCrawler` hand swap, `bCanClimbLadders` false, smaller collider), and the eye height follows walkType 21/22 client-side. All of it runs in the client's own damage-response path off the package this server now sends (`ProcessPackage` IL_0255+ re-runs `ProcessDamageResponse` locally), and the walkType itself never crosses the wire - the client derives spawn walkType from its own entityclasses copy (`GetSpawnWalkType`) and the transitions from the damage bits. Server-side there is no ladder-climbing model for the crawler ban to bite into, and no speed branch to mirror. So with the roll, the state, and the S2C carry all landed, the observable behavior is complete: the client's zombies limp, crawl, and lose limbs off zdtd's numbers. The 143 fold landed 2026-09-06 (`Game.dismemberSelfChance`): purchased attribute/perk `DismemberSelfChance` rows fold at level via curveAt plus the actor's active-buff rows at level 1 (base_set overrides, base_add/subtract accumulate, perc ops skipped - no stock 143 row uses them), revertible by recompute-from-set like the buff VM. Status moves to SHIPPED with the one stated dormancy above (the crawler `HandItemCrawler` hand swap, the crawler `HandItemCrawler` hand swap, whose buff table overlaps the general server-side buff-application gap rather than adding a crawler-specific one - zombie melee damage here resolves off class `attack_damage` either way) |
...
| `NetPackageEmitSmell` | N/A headless (re-audited 2026-09-02: no handler and no emitter. Stock smell is a client-side AI stimulus; the server-side consumer would be zombie sense, which reads its own sim state rather than a client claim - accepting one would let a client inject AI stimulus (rule 17, server authoritative). Evidence it is not needed: a full smoke-navezgane session logged zero `c2s_unhandled` packages, so the stock client never sends it to us) |
...
| `NetPackageAnimateBlock` / `BlockTrigger` | WORKS, with a documented divergence in the rebroadcast (DIVERGENCES 3, 2026-09-04): stock's `BlockTrigger` is ToServer and its ProcessPackage ends in `TriggerManager::TriggerBlocks` without emitting anything, so a stock dedi never forwards it; zdtd broadcasts it to nearby peers instead. `NetPackageAnimateBlock`'s only stock sender is the GameEvent block-animate action (ActionBlockAnimateBlock) - zero stock GameEvent data uses it, so a stock dedi never sends it (parity N/A, same dormant GameEvent surface as the game-events row) |
...
| `NetPackageGameMessage` / tips | WORKS (re-audited 2026-09-02: the C2S handler validates the stock body (write IL=17: msgType u8 EnumGameMessages, mainEntityId i32, secondaryEntityId i32) and re-broadcasts it verbatim to every client, byte-identical to stock `FinishGameMessageServer` IL=69 including the echo to the sender. Rate-gated on the inv bucket; covered by a scenario test. The P2 label predated the handler) |
...
| Projectile / ranged combat | WORKS (2026-08-20, RE ../../7dtd-engine-research/docs/gameplay/items.md:1097-1140: projectiles are client-side GameObjects with ProjectileMoveScript, never server entities; the server surface is the C2S NetPackageDamageEntity claim, which zdtd validates range/cap/fatal/PvP/armor, applies, knocks back and kills) |
...
| Death / backpack | WORKS (server spawns the death bag on the lethal event - C2S damage and hp-replicate AI-kill paths both call spawnDeathBag - with the victim's real inventory range by DropOnDeath mode (1 all, 2 toolbelt, 3 backpack) at preserved offsets, the "Backpack" entity class (EntityBackpack, not the DroppedLootContainer the block spills use; the class survives a restart via the `zen_rec_backpack` tag), and the backpack map pin relayed; status.md DropOnDeath row) |
| Plugin spawn reversion (paper 3.1) | WORKS (2026-08-28: the command buffer records each applied spawn per plugin src, and withdrawal - self-disable and admin reload - despawns those entities so a module's zombies do not outlive it; ring capped at max_commands, oldest attribution dropped when full; ADR 0030 amended. 2026-09-10: the reversion is now visible to clients. Both plugin destroy paths - the withdrawal above and the `despawn` queue verb - destroyed the entity without a `NetPackageEntityRemove`, so the model stood on every client's map for the rest of the session (nothing else ever names that entity again). Every other destroy path in the tick already reported: the corpse sweep refuses to destroy past its report cap rather than leave a ghost. The verb reports through its own `cmd_despawned_*` list on the tick result, since the command drain is not a system and does not ride the far-despawn sweep's) |
...
| Challenges system | WORKS (2026-08-21: challenges are client-tracked per RE quests-challenges.md §5 - the client loads challenges.xml locally and advances objectives via its own event hub; the server surface is the challengegroup_reward_* quests (recognized by isStockClientQuestName, accepted via the generic quest accept, completed through the quest system - the catalog covers all stock defs) plus NetPackageGameEventRequest acks. Corrected 2026-09-06: "no server-required challenge wire exists" was wrong. The client's event hub does not observe a remote kill on its own - `NetPackageEntityAwardKillServer` is what makes `QuestEventManager.EntityKilled` fire there, and `ChallengeObjectiveKill` / `ChallengeObjectiveKillByTag` subscribe to that event. zdtd sends it since the same date, so kill challenges advance; the rest of the row stands) |
...
- **Allies.** `NetPackageAllyRequest` (asm.il 886226) is decoded, the sender is
  ...
  ../../7dtd-engine-research/docs/admin/server-lifecycle.md:299), which zdtd does not reproduce field-for-field.
