...
GAP_ANALYSIS "Join-burst tick budget under concurrent load", PARTIAL).
Counts unchanged.
...
payload - the PARTIAL label was stale.
Then the EntityVelocity/EntityPhysics row was re-scoped (2026-08-26):
EntityPhysics is the physics-master optimization
(PhysicsMasterSetupBroadcast IL=31, view-entry send gated on >0.05u / 1 deg
motion, one client runs the physics) - a client-interpolation nicety, not an
authoritative sim channel; the PosAndRot frames carry the motion and the
momentum ragdoll is client-visual, so the residual is recorded not blocking.
...
re-arm all ship; the residuals (sleeper-pose byte table, CanSpawn 2.1 gate)
were already documented elsewhere.
...
discretionary execution policy is plugin-owned, while the stock-required
death/respawn subset runs native from `gameevents.xml`
(`server/game/game_events.zig`; unsupported sequences fail closed).
No general 132-verb GameEvent phase machine is built.
...
Then the sleeper-volume-coverage row went WORKS: the volume-store ref build
is uncapped beyond the max_volumes budget (8192) - pass 2 previously broke at
1200 refs, leaving a few hundred far POIs with no sleepers; now every POI's
...
Then the InvTx-wire row went WORKS: the stock InventoryTransaction ops now
apply to the player's ECS inventory (SetAbsolute/SetRelative write the
reported stack at the index, SetAll replaces the array, capped and
fail-closed) with the stock minimal response ack; the handler tries the
stock layout before the native 11-byte body (which only checked len >= 11
and would misread stock traffic), and the Guid registry path stays unpinned
...
pushes stage 15 to 51 for a level-10/5-day player. Test active quest stage
modifiers scale the player gamestage. Gamestage row stays PARTIAL for the
prefab DifficultyTier and EffectManager passives.
...
13x13 disk with no server terrain beyond ~104 blocks. The default
view-7 now streams its full 225 chunks, chunk_stream_radius_max defaults
to 12, and the in-view bitset grew to 640 bits so the O(1) membership
probe stays on the fast path. World 38/6/0; total **254/37/0**.
...
bodies all match the pinned wire. A second pass (2026-08-27) verified
the HUD/movement batch against the IL too: game-stats (i16 length prefix
...
4. Stealth / crouch (PARTIAL - crouch replicates (flags bit 512), hearing muffled 0.5x; movement-noise volume model wired 2026-08-26 (sounds.xml `<Noise>` table + PlayerStealth fold: NotifyNoise accumulation, CalcVolume, sleeper wake at 360, heat, attraction heard-test; stock-dedi inputs are server-side sounds only - the C2S sound relay is audio-only on a dedi); the sleeper crouch leg uses the stock FastLerp(3,15,lightAttackPercent) with lightAttackPercent = passive-89 when selfLight (held-item light) < 0.1 per the exact TickServer IL_010B; since 2026-08-27 the held item's items.xml LightValue is parsed and a held light (torch/flashlight) lifts the switch to 1, so the crouch reach stretches to the full FastLerp(3, 15, t) range (`[rules.ai] crouch_sleeper_detect_min/max` + `stealth_light_passive`); CanSeeStealth sight-light leg SHIPS 2026-08-26 (TickServer lightLevel chain → S2C light byte + sight gate vs per-class SightLightThreshold, night blinds sight except point blank, crouch shrinks the lit reach); sleeper wake light gate SHIPS 2026-08-26 (GetSleeperDisturbedLevel IL=38: wake = Lerp(rolled SleeperSightToWakeMin/Max, dist/sightRangeBase) vs the player lightLevel, per-entity deterministic rolls at spawn - POI sleepers wake within ~15% of sightRange at noon, ~1 m at night); sleeper stir SHIPS 2026-08-26 (an in-volume player that does not wake the sleeper triggers SetSleeperActive one-shot + NetPackageSleeperPassiveChange so the client plays the groan); block light + shade + the light-refined groan threshold + the noise-to-sense ladder recorded; the MOON TERM SHIPS 2026-08-27, the selfLight HELD-ITEM TERM SHIPS 2026-08-27 and the speedAverage MOVEMENT-VISIBILITY TERM SHIPS 2026-08-27 (per-tick horizontal speed lerp/decay, light x (1 + speedAverage x 0.15), IL_00CD) (items.xml LightValue parsed per item - torch .35, flashlight .55, gun lights .45; Inventory.GetLightLevel IL=76 pin; folded into the stealth light blend + the lightAttackPercent crouch-reach switch) (weather-environment.md section 5 pin: the 7-phase sMoonPhases/sMoonBrights tables extracted from the PE via Mono.Cecil, moonBright = sMoonBrights[((int)(day+5.5)) % 7], GetMoonAmbientScale FastLerp fold, wired into step.zig ambient - night stealth light now varies with the moon phase))
...
8. World borders / difficulty tiers (WORKS 2026-08-26 - the six difficulty presets were extracted from the stock client bundle (UnityPy, sandbox-options.md section 3) and comptime-parsed into [rules.difficulty]; the border is client-side by design; see the GAP world-borders/difficulty-tiers row)
9. Server-triggered sounds / music (WORKS 2026-08-27 - NetPackageSoundAtPosition wire pinned (5 fields, volumeScale Setup-only) and the C2S relay ships with owner/rate/NaN gates; see the GAP sounds row)
...
| Parity batch 2026-07-23 | **PASS (partial cores)** | POI sleeper volumes from prefab .tts/.nim, blood-moon BloodmoonMusic builder (HordeEvent builder unwired: stock has no sender - re-audited WORKS/N-A 2026-08-26, no Setup callers in the full dump), electrical block placement + WireActions (WireToolActions visual handshake re-audited WORKS 2026-08-26 - peer rebroadcast is the stock behavior, ProcessPackage IL=254 has no graph mutation; the graph edits ride WireActions which the server applies), vehicle terrain gravity/ground-clamp, trader stock TraderData wire, quest multi-phase objective graphs, EAI prioritized task graphs, in-game console commands. All PARTIAL with documented gaps (GAP_ANALYSIS.md) |
