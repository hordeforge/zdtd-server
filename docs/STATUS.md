...
the loot stage (84 stock ammo rows). The residual is the remaining loot-entry
attributes (`tags` for the sandbox count/prob paths, `buffs` to the opener,
`random_durability`, `mods`); the row stays PARTIAL for those.
