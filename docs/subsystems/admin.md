...
The F1 console is a separate, narrow surface. A read-only allowlist (`gettime`, `listplayers`, `listplayerids`, `listents`, `say`, `version`, and the client-side menu verbs) runs for any player and is answered with `NetPackageConsoleCmdClient` lines; every other verb requires an admin list entry, and then a per-command required level that must not be more privileged than the caller before the line is routed through `runAdminLine` (`src/server/admin_console.zig:291`, `:307`). `permLevelOf` returns the stored list level, else the 1000 default (`src/server/game.zig:3274`). Platform identity is checked first (stock `AdminUsers.HasEntry` IL=30); a name-keyed entry applies only when the client presented no platform id, so a spoofed display name cannot mint admin rights. `commandLevel` returns 0 for a verb with no entry (`src/server/game.zig:3358`), so an unconfigured verb is top-admin-only. In-game `admin add` also refuses granting a more privileged level than the actor (`admin_actor_level`). `cp` settings live in `Game.command_levels` (`src/server/game.zig:782`) and are never written to disk, so they do not survive a restart.
...
The stock `serveradmin.xml` merges into the same three lists at startup (`src/server/admin_xml.zig:140`), keyed by `platform` + `userid` with the legacy `steamID` fallback (`src/server/admin_xml.zig:37`). `tickServerAdminReload` re-checks the file every 5 s and re-applies it, clearing only the XML-sourced entries (`src/server/game/tick.zig:2759`). `wipeplayer` erases a player record by login name, dropping any online session first, releasing that name's container claims, and removing ally entries keyed on the dropped identities (`src/server/admin_console.zig:1458`); ally relationships themselves live in `src/server/ally.zig` and persist to `{world_dir}/allies.zal` (`src/server/ally.zig:20`).
...
Fail-closed rules are explicit: `listen` rejects a non-loopback bind and an over-long token (`src/server/mcp_transport.zig:76`), the CLI rejects a non-loopback `--mcp-bind` with a message naming a TLS reverse proxy as the remote path (`src/main.zig:691`), only `POST /mcp` is served (`src/server/mcp_transport.zig:205`), a non-empty token is compared in constant time and a miss is 401 with `WWW-Authenticate: Bearer realm="zdtd-mcp"` (`:221`), a non-`application/json` Content-Type is 415 (`:229`), `Transfer-Encoding` is refused rather than decoded (`:217`), and the frame cap is 16 KiB inbound with an 8 KiB response buffer (`:24`, `:26`). A guest that returns zero bytes is answered `202 Accepted` with no body, the MCP notification shape, never a fabricated response (`:249`). The handler is a function pointer supplied by `Game`; the frame is routed to the first *live* plugin exporting `on_mcp_frame`, so a trapped module's empty reply cannot swallow the frame forever (`src/server/game/wasm_host.zig:416`). The allowlist for the guest's `admin_command` tool is held as the `mcp_allowlist` string on `Game` (`src/server/game.zig:697`) and served to the guest one prefix per line through the `zdtd.query` verb `mcp.allowlist` (`src/server/game/wasm_host.zig:427`). `deinit` clears the token and nulls the frame handler so a poll after teardown finds no handler rather than a dangling one (`src/server/mcp_transport.zig:87`). Token may also come from env `ZDTD_MCP_TOKEN` when `--mcp-token` is empty (`src/main.zig:633`).
