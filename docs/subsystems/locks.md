...
A channel lock is a lease, so the path refuses by default. The server clamps the requested channel into its table and reaps a stale holder whose keep-open stamp aged out (`src/server/c2s/misc.zig:1058`, `:1060`). It refuses a peer that already holds any channel, force-unlocking what that peer holds and granting nothing, which is stock's gate 1 (`src/server/c2s/misc.zig:1069`). A null or over-long target list is denied explicitly so the client's pending request resolves instead of hanging, a dead or closed trader is refused before any channel is written, and a target locked elsewhere or a channel held by another peer is refused as `locked` (`src/server/c2s/misc.zig:1083`, `:1131`, `:1139`). An unlock is accepted only from the holder or when the channel is free, and it runs the tile-entity close consequences (`src/server/c2s/misc.zig:1216`). A keep-open refresh carries no state and only restamps the timer (`src/server/game/locks.zig:100`).
