...
Two stores are reachable only through this ladder: `traders.zst` (`src/server/persist.zig:63`) and the two sleeper files (`src/server/persist.zig:64`). That is wider than the periodic autosave in `Game.step`, which calls neither (`src/server/game/step.zig:523`), and wider than shutdown, which calls neither (`src/server/game/lifecycle.zig:9`). Trader stock and cleared sleeper volumes therefore persist only when an operator runs `.save` or `.saveworld`, or when a test calls `Game.saveTraders` directly (`src/server/scenarios.zig:10845`). Treat that as a gap in the restart contract, not as durability.
...
The periodic save is a modulo on the tick counter, and it is the primary writer of everything except the two stores noted above (`src/server/game/step.zig:523`):
...
`save_interval_ticks` defaults to 100 ticks (`src/server/game/types.zig:91`) and is a bound field, so `[stream] save_interval_ticks` in `zdtd.toml` overrides it without a parse arm (`src/server/game/types.zig:370`). The player save inside that block is gated on `players_dirty`, which is set by the character-sheet update path (`src/server/c2s/misc.zig:353`) and cleared when the save runs (`src/server/game/step.zig:541`). The block is instrumented with the `save_io` and `save_encode` apm sections because it runs on the tick.

Per-event saves cover state a periodic write could lose: a dropped or reaped session saves that player before its data disappears (`src/server/game/session_drop.zig:28`, `src/server/game/tick.zig:1545`). Shutdown saves players first and flushes chunks before anything else, waits on the background chunk writer, then drains the remaining stores and tears the process down (`src/server/game/lifecycle.zig:9`), which is the full save a graceful `.shutdown` performs, and the bounded `--once` run additionally flushes chunks before it reports (`src/main.zig:1157`). Admin `.save` and `.saveworld` both run the full `saveAllStores` ladder and reply with the ladder's own success value (`src/server/admin_console.zig:1310`, `src/server/admin_console.zig:1761`).
