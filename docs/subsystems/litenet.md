...
Two properties are handled outside the game layer: a `ping` is answered with an 11-byte `pong` (property, sequence, `i64` ticks in 100 ns units), and an `mtu_check` is echoed at its own length as `mtu_ok` (src/litenet/peer.zig:626, src/litenet/peer.zig:642). A wrong pong size fails the client's own packet verification, which is why the length is pinned rather than approximated. `broadcast`, `unconnected_message`, `peer_not_found`, `invalid_protocol` and `nat_message` fall through the `else` arm and are dropped, and no LiteNet-level encryption or NAT punch path exists in these files (src/litenet/peer.zig:662).
