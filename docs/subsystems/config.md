...
`main.zig` is the only place the order is realized. `--serverconfig` is loaded first and a missing or unreadable path is fatal (`src/main.zig:512`); the config is copied into the `InitOptions` struct literal field by field (`src/main.zig:645`). `zdtd.toml` is looked up as `<world_dir>/zdtd.toml` then CWD `zdtd.toml`, first existing file wins (`src/main.zig:730`), and an unreadable one aborts startup (`src/main.zig:740`). Preset selection is `--preset`, else `[preset] name` from the already-loaded `zdtd.toml` (`src/server/zdtd_config.zig:202`, resolved at `src/main.zig:749`); a config-only mod's own preset sits below an explicit pick (`src/main.zig:828`). The published order is `defaults < serverconfig < preset < zdtd.toml < CLI` (`src/main.zig:874`), which extends the header order in `zdtd_config.zig` that omits the preset tier (`src/server/zdtd_config.zig:2`). A preset or `zdtd.toml` declaring a name that does not match its filename is fatal (`src/main.zig:864`).
