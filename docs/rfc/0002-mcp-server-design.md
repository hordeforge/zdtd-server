...
| Cap | Value | Note |
|---|---|---|
| `max_frame` | 16 KiB | inbound JSON-RPC body (RFC 0002 `max_frame_kib`) |
...
1. **Guest protocol unit tests** (host-side harness loading the committed
   `mods/mcp/mcp.wasm` through `WasmHost`): initialize/ping/
...
4. Gates: `zig build test` and `make check` stay green. No loadgen/stock
   client leg (no stock wire; PRD 0002 NFR-5, ADR 0031 D7).
