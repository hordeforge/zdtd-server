...
|---|---|---|
| G1 stock dedi mod assetbundle load | `Mod` / `AssetBundles` IL in `../../../7dtd-engine-research` (`Mod.LoadBundles` if present, failure handling) | Document: zdtd never reads `Bundles/`; bundle content is client rendering (R11) |
...
| G4/G5/G6 csvoperations / conditional / include grammar | `XmlPatchMethods` IL dumps in `../../../7dtd-engine-research/tools` | Implement documented common forms, unit-test, mark unverified; stock-client smoke as witness |

Verification per gap: `../../../7dtd-engine-research` doc or `il/` line cited in the
implementation comment; else the fallback note lands in [PRD 0003](../prd/0003-modlets.md) §8.
...
1. **`src/assets/modlets.zig` (new)** - stock ModManager subset for XML-only
   ...
   - expose `config_dirs: []const []const u8` (each mod's `Config/` dir, mod
     order), mod names/paths for `@modfolder(Name)` lookup, bundle tolerance
     (logged once, never read), code-mod warnings (DLLs present: loud warn, XML
     still applies);
...
3. **`src/assets/paths.zig`** - `setOverrideDirs` called from `init_assets` with
   `mods.config_dirs ++ opts.config_overrides`; keep the `.zdtd_cfg_cache` merge
...
2. **Localization (R10, gated on G8)** - merge each mod's `Localization.csv`
   ...
   `totalParts` scheme, `prepareDataPackets` IL=107) is closed in `../../../7dtd-engine-research`.
...
[PRD 0003](../prd/0003-modlets.md) §8 either way.
