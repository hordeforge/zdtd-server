...
**Ships as:** a Wasm plugin module (ADR 0020 / ADR 0026) + a small host
sense/act surface.
**Behavioural reference:** `../7dtd-clanker` (the stock-server FPS bot C# mod),
...
ecosystem has a bot mod (`../7dtd-clanker`), but it is a **C# stock-host mod** —
...
- As an **operator**, I can add `mods/zdtd_bot.wasm` to `[plugin] modules` and
