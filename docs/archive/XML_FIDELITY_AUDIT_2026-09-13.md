...
data a dedicated server ever sends. **Open from this pass:** a writable crate's
composite carries storage AND signable (and lockable), and the storage echo is
built storage-only, while the client's `TileEntityComposite::read` walks the
block's declared module order and abandons the whole payload on a hash
mismatch. The faithful echo re-encodes every module in that order (storage from
the server's clamped container, signable from the received text, lockable from
the lock state), which needs the per-block `CompositeFeatures` name list from
blocks.xml rather than the signable flag alone. Everything else below is still
queued.
