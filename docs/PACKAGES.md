...
**Totals:** 190 packages; 33 ToServer (all handled: True); handled cases 69; S2C emitted 47.
...
| NetPackageEntitySpawn | ToClient |  | sent (zombie+item-drop) | `NetPackageEntityTargeted.read;EntityCreationData.read;` (ECD = header + entityClass switch + networkWrite tail; item-drop itemClass branch done) |
