...
| 6 | Starter kit (4 items + counts) hardcoded in `ecs/world.zig` spawnPlayer | none - stock defines the kit in code, so it is server policy | moved to config (ADR 0010): `[sim] spawn_starter_kit` (rows `name` or `name:count`) is parsed once at `Game.create` after the catalog loads, resolved through items.xml, unknown names omitted (fail closed), counts clamped to Stacknumber; unset keeps the built-in default kit. The earlier claim that this had moved, when the code was still hardcoded, was corrected 2026-09-12 (hardcode audit B1) |
