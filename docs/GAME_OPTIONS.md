...
| `crouch_sleeper_detect_min` / `crouch_sleeper_detect_max` | 3.0 / 15.0 | Sleeper attack-detect range while the target crouches: `FastLerp(min, max, lightAttackPercent)` (RE entity-ai.md `CanSleeperAttackDetect`, stock bounds 3..15). The t comes from the slice-1 day/night ambient: 0.89 passive in deep dark (`stealth_light_passive`), else 1 |
| `stealth_light_passive` | 0.89 | `PlayerStealth.TickServer` passive-89 fold for `lightAttackPercent` in deep-dark ambient (< 0.1). Stock `EffectManager.GetValue(Passive89)` with no items = 0.89; servers tune the night stealth here |
