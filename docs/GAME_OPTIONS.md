...
|---|---|---|
| `[stream]` | `max_streamed_chunks`, `stream_radius_min/max`, `chunk_adds_per_stream_tick`, `chunk_stream_period_ticks`, `motion_replicate_period_ticks`, `pos_heartbeat_period_ticks`, `world_time_send_ticks`, `vehicle_pos_send_ticks`, `sleeper_tick_ticks`, `turret_sync_ticks`, `save_interval_ticks`, `spawn_area_radius_max` | Chunk stream caps (clamped to compile cap 169) + broadcast/side-work cadences in ticks. `pos_heartbeat_period_ticks` (default 5) is the PosAndRot heartbeat for an entity with no dirty motion bit (paired with `motion_replicate_period_ticks`; the effective interval is their LCM), clamped to >= 1. `sleeper_tick_ticks` gates prefab sleeper volumes, airdrops and workstations; `turret_sync_ticks` gates turret state broadcasts; `save_interval_ticks` gates the periodic world flush |
| `[authority]` | `interest_range_blocks`, `max_edit_range_blocks`, `max_horizontal_speed_mps`, `max_vertical_speed_mps`, `max_claimed_damage`, `max_claimed_noise_scale`, `peer_stale_ms`, `lock_stale_ms`, `join_rate_limit_ms`, `mode`, `guard_enforce`, `guard_dry_run`, `guard_quarantine`, `guard_load_shed`, `guard_window_ticks`, `guard_strong_distinct`, `guard_hard_repeat`, `guard_kick_delay_ticks`, `guard_shed_hold_ticks`, `guard_weak_break_rate` | C2S range / interest / mode. `join_rate_limit_ms` (default 500) paces connection attempts per IP like stock's ~500 ms/IP flood gate; loopback is exempt so bots and tests share 127.0.0.1. `lock_stale_ms` (default 10 000 ms = 10 s) auto-releases a container/trade lock after the last grant or keep-open refresh. Stock's `LockManager.Update` (IL=128) force-unlocks a `keepOpenTimes` stamp older than 10 s, and the client refreshes it every 2.5 s with `NetPackageInventoryKeepOpen` (handled in `c2s/misc.zig`) while its window is open, so the value is the silence window rather than a lock lifetime. `max_vertical_speed_mps` (default 25) caps the Y delta of the movement envelope (rejects fly-hacking teleports the horizontal clamp cannot see; stock jump/fall are ~7.5/9.8 m/s). `max_claimed_noise_scale` (default 8) caps the `volumeScale` a client attaches to `NetPackageAudio`: stock multiplies the clip's sounds.xml noise volume by it before `AIDirector::NotifyNoise`, so an unbounded claim would turn one footstep into a server-wide alarm. The ADR 0037 glide keys live in `[rules.glide]` (see the `[rules]` table): `sink_vy_mps` (default 2.5) clamps the C2S vertical delta while a player's glide flag is armed (server-side deceleration, position broadcast back) and `item_tag` (default `parachute`) drives the sense v4 `wearing_glider` bit for armor-slot items. The `guard_*` keys are the P4 guard policy (defaults log-only, dry-run on); `guard_kick_delay_ticks` (default 10 = stock `disconnectLater(0.5f)`), `guard_shed_hold_ticks` (default 40 = 2 s valve hold) and `guard_weak_break_rate` (default 900 destroys/window) tune the policy ladders; see [AUTHORITY.md](AUTHORITY.md) |
...
| `[plugin]` | `deny`, `allow` | Queued-verb interception (ADR 0039): `module=verb,verb; module2=verb` lists. `deny` refuses those commands for the named module, `allow` re-allows a verb the module's own `manifest.toml deny` refused (the operator context is right-biased). Known verbs: `spawn`, `despawn`, `damage`, `say`, `glide`, `bot`; a bad entry or unknown verb fails startup. The module key matches the manifest name, the `.wasm` file stem or the mod directory name. Denied commands are dropped before the command buffer and counted in `plugin_verbs_denied` |
...
| `fall_sink_vy_mps` | 0 | Global player fall sink (blocks/s) applied WITHOUT the glide flag: > 0 clamps every player's C2S vertical delta (server-side fall slow-down; the moon_gravity mod sets its lunar terminal velocity). 0 = stock envelope |
| `[rules.ai]` | | |
...
| `crouch_sleeper_detect_min` / `crouch_sleeper_detect_max` | 3.0 / 15.0 | Sleeper attack-detect range while the target crouches: `FastLerp(min, max, lightAttackPercent)` (RE entity-ai.md `CanSleeperAttackDetect`, stock bounds 3..15). The t is `lightAttackPercent` = 0.89 passive (`stealth_light_passive`) when the player's selfLight (held-item light) < 0.1, else 1 - no item-light model, so the passive always applies |
| `stealth_light_passive` | 0.89 | `PlayerStealth.TickServer` passive-89 fold for `lightAttackPercent` when selfLight (held-item light) < 0.1. Stock `EffectManager.GetValue(Passive89)` with no items = 0.89; servers tune the night stealth here |
