...
**Join map:** `packages.default_mappings` has **191** names (runtime `package_maps=191`; the 3.2.0 additions - ConfirmSpawnEntity, POIMetadataRequest, POIMetadataResponse - lifted the count from the 190 pinned in the original parity run, and the invented `NetPackageEntityFlags` name was removed 2026-08-29 - not a stock class, the real package is NetPackageEntityAliveFlags; a stock client rejects unknown names with EKickReason 18). 7 mapped names are not yet in this generated table - the four the parity run noted (NetPackageDroneDataSync, NetPackageDroneParticleEffect, NetPackageLight, NetPackageTreeFade - all real classes with bodies in the research corpus) plus ConfirmSpawnEntity, POIMetadataRequest, POIMetadataResponse; conversely `NetPackagePOIAround` still appears in the table but is removed in 3.2.0 (replaced by POIMetadata, see the delta section above). Regenerate via the parity tooling below rather than hand-editing. NOTE: the parity tooling's coverage mode is not implemented in the current parity_diff.py - the live dispatch/S2C counts on line 9 were a manual count (2026-08-29); see PARITY_TOOLING.md.
