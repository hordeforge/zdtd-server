...
Reconnect and wipe need a stable key for `players.zsv` (ZPV2). Stock clients expose a login display name early in join; platform/EOS ids are optional, version-sensitive, and not fully wired on the EAC-off research path.

## Decision

1. **Primary key = login name** (UTF-8, length-capped on wire/save). Merge-write and `wipeplayer <name>` match on this string.
2. **No platform id column in ZPV2** until a RE-backed, version-stable id is required for multi-name or rename cases.
...
- Adding platform id later is a **ZPV3** (or flagged extension) migration, not a silent field insert.
